	...
	for(int i = m_rcClient.left - 3; i < m_rcClient.right + 2 && i < iDataLength; i++)
 	{
		if(i < iHead)
			continue;		
		y1 = (int)((m_center)-(int)((((((pDoc->m_ADBuffer[i]/*^0x800*/)&0xFFFF)-LSB_HALF-m_MiddleLsb)))
			*m_rcClient.Height())/ 65536);
		y2 = (int)((m_center)-(int)((((((pDoc->m_ADBuffer[i+1]/*^0x800*/)&0xFFFF)-LSB_HALF-m_MiddleLsb)))
			*m_rcClient.Height())/65536);
		pDC->MoveTo(i, y1); 
		pDC->LineTo(i + 1, y2);
	}		
