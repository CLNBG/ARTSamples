...
int CCfgTabCtrlBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	//m_StartDeviceButton = new CButton();
	m_StartDeviceButton.Create(L"开始数据采集", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON | BS_DEFPUSHBUTTON , StartButtonRect, this, IDM_StartDeviceAD);
	//m_StopDeviceButton = new CButton();
	m_StopDeviceButton.Create(L"停止数据采集", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON  , StopButtonRect, this, IDM_StopDeviceAD);
...
void CCfgTabCtrlBar::OnSize(UINT nType, int cx, int cy) 
{
	...
	int bottom = (IsHorz() || IsFloating()) ? cy - 64 : cy - 88;
	...
	for (POSITION pos=m_views.GetHeadPosition(); pos; m_views.GetNext(pos))
	{
		...
		pWnd->MoveWindow(9, 99, cx-15, bottom-26); // 在CControlBar在移动视图类的控件位置
	}
}
...
{	
	...
	CString str = lpszLabel;
	memcpy(pMember->szLabel,str,str.GetLength());
...
/*void CCfgTabCtrlBar::OnKillFocus(CWnd* pNewWnd) 
{


	CCfgCtrlBar::OnKillFocus(pNewWnd);
