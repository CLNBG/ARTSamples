...
int CADFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_pWndSTCBar->Create(this, CSize(230, 100), IDW_PROP_BAR);
	...
	if(m_ProcessMoveVolt == 1) // 开始求中间值
	{
		...
		if (gl_ReadSizeWords>MAX_RAM_SIZE)
		{
			...
			for(int nChannel=0; nChannel<MAX_CHANNEL_COUNT; nChannel++)
			{
				...
				for(ULONG Index=0; Index<MAX_RAM_SIZE/MAX_CHANNEL_COUNT; Index++)
				{
					DataAnalyze[nChannel][Index] = ((ADBuffer[nChannel][glBufferID][Index])&0xFFFF);
				}
			}
			/////////////////////////////////////////////			
			for(int k=0; k<MAX_CHANNEL_COUNT; k++)//每通道的数据各自进行排序
			{
				m_QSortData.Sort(DataAnalyze[k], 0, MAX_RAM_SIZE/MAX_CHANNEL_COUNT);
		...
		else
		{
			...
			for(int k=0; k<MAX_CHANNEL_COUNT; k++)//每通道的数据各自进行排序
			{
				m_QSortData.Sort(DataAnalyze[k], 0, gl_ReadSizeWords/MAX_CHANNEL_COUNT);
				m_DataMiddle = m_QSortData.GetMidData(); // 取得中间数		
				// m_DataMiddle是非偏移码，即原码，即0-LSB_COUNT中的任意值
				// gl_MiddleLsb 是偏移码值，即原码减去LSB_HALF后的值,即在原点上下浮动的值
				// gl_MiddleLsb[k]=(int)((m_DataMiddle)-LSB_HALF);
...
void CADFrame::OnDestroy() 
{
	...
	// TODO: Add your message handler code here
	PCI8501_SaveParaAD(NULL, &ADPara);
	...
	;
	CSysApp* pApp = (CSysApp*)AfxGetApp();
//	PCI8501_ReleaseDevice(pApp->m_hDevice);
