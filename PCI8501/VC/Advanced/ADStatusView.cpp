...
void CADStatusView::OnSelchangeComboScreenVolume() 
{
	...
	if(pWaveView->m_ADScopeCtrl.m_bAllChannel == FALSE) //单通道显示时才能测电压
	{
			...
			if (ADPara.InputRange[g_nChannelNum]==PCI8501_INPUT_0_P10000mV||ADPara.InputRange[g_nChannelNum]==PCI8501_INPUT_0_P5000mV)
		     { 
			  pWaveView->m_ADScopeCtrl.SetRange(0, gl_ScreenVolume, pWaveView->m_ADScopeCtrl.m_nChannelNum);
		     }
		    else
		     {
