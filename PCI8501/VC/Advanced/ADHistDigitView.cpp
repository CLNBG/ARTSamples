...
void CADHistDigitView::OnInitialUpdate() 
{
	...
	str.Format(L"设备精度:%d位", pDoc->m_Header.DataWidth);
	pList->AddString(str);
	...
	str.Format(L"M_Length:%d", pDoc->m_Header.ADPara.M_Length);
	pList->AddString (str); 
	str.Format(L"触发事件提前:%d", pDoc->m_Header.ADStatus.bAheadTrig);
	pList->AddString (str);
...
//	List.SetRedraw(FALSE);
	...
	{		
		...
		for(Col=0; Col<(UINT)1/*pDoc->m_ChannelCount*/; Col++)  
		{
			...
			{
				case 1://十进制显示
					swprintf_s(str,L"%d", (pDoc->m_ADBuffer[Row*1+Col]/*^0x800*/)&0xFFFF);
					break;
				case 2://十六进制显示
					swprintf_s(str,L"%5x", (pDoc->m_ADBuffer[Row*1+Col])&0xFFFF);	
					break;
				case 3:
					swprintf_s(str,L"%d", (int)((((pDoc->m_ADBuffer[Row*1+Col]/*^0x800*/)&0xFFFF)-LSB_HALF)*m_PerLsbVolt));	
					break;
			}
