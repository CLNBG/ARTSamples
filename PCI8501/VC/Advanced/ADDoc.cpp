...
#endif
#ifdef PCI8501_CreateSystemEvent
#define PCI8501_CreateSystemEvent PCI8501_TCreateSystemEvent 
#endif
////////////////////////////////////////////////////////////////////////
WORD ADBuffer1[MAX_CHANNEL_COUNT][MAX_SEGMENT][MAX_RAM_SIZE]; // 缓冲队列
...
PCI8501_PARA_AD ADPara;
PCI8501_PARA_AD OldADPara;
PCI8501_STATUS_AD gl_ADStatus;
...
HANDLE hFileObject[MAX_CHANNEL_COUNT];
...
CString g_strFileFullName[MAX_CHANNEL_COUNT];
ULONG gl_StartAddr;
//##############################################
...
CADDoc::CADDoc()
{
	...
	gl_nChannelCount=0;
	PCI8501_LoadParaAD(NULL, &ADPara);  // 读入系统参数
...
void CADDoc::StartDeviceAD() 
{
	...
	FileLenghtWords = sizeof(FILE_HEADER); // 文件长度初始化
	...
	// 创建数据采集与窗口绘制同步事件对象
	hEvent = PCI8501_CreateSystemEvent();
	hExitEvent = PCI8501_CreateSystemEvent();
	...
	if(!m_bFirstCreateDeviceFile) // 如果创建了文件对象
	{
		for (int Index=0; Index<MAX_CHANNEL_COUNT; Index++)
		{
			...
			case PCI8501_INPUT_0_P5000mV: // 0～5000mV
				...
				header[Index].VoltBottomRange = 0;
				break;
			...
			case PCI8501_INPUT_N2500_P2500mV: // ±2500毫伏
				header[Index].VoltTopRange = 2500;
				header[Index].VoltBottomRange = -2500;
				break;
			...
			header[Index].ChannelNum = Index; // 通道号
			...
			//PCI8501_WriteFile(hFileObject[Index], (WORD*)&header[Index], header[Index].HeadSizeBytes);
...
//	SetEvent(hExitEvent);
	...
	PCI8501_ReleaseSystemEvent(hEvent);	   // 释放消息
	// 终止采集线程
	CProgressCtrl* pProgress=(CProgressCtrl*)(pShow->m_wndShowStatus.GetDlgItem(IDC_PROGRESS));
	pProgress->SetPos(0);  // 释放设备时，置进度条为起值
...
void CADDoc::OnDataFileNew() 
{
	...
	{  // 如果不是第一次创建文件对象,则释放老文件对象
		...
		{			
			//if ( ! PCI8501_ReleaseFile( hFileObject[nCH] ) )
	...
	{		
		CString str;
		str.Format(L"-%d.pxi", nCH);
		strNewFileName = TempFileName + str;
		CFile ff;
		hFileObject[nCH] = CreateFile(strNewFileName, GENERIC_READ|GENERIC_WRITE, 0, NULL,
			CREATE_ALWAYS,
			FILE_ATTRIBUTE_NORMAL, NULL);
		//hFileObject[nCH] =  PCI8501_CreateFileObject( pApp->m_hDevice, strNewFileName, PCI8501_modeCreate|PCI8501_modeWrite ) ;
	...
	GetDiskFreeSpaceEx(strNewFileName.Left(3), NULL, NULL, &uLarge);
	ULONGLONG DiskFreeBytes=uLarge.QuadPart;  // 取得文件所在磁盘可用空间
...
void CADDoc::OnCloseFile() 
{
	...
	for(int nChannel=0; nChannel<MAX_CHANNEL_COUNT; nChannel++)
	{
		...
		if (!WriteFile(hFileObject[nChannel], (WORD*)&header[nChannel], sizeof(FILE_HEADER), &dTmp, NULL))
		{
			AfxMessageBox(L"error");
		}
		CloseHandle(hFileObject[nChannel]);
		//PCI8501_SetFileOffset(hFileObject[nChannel], 0);
		//PCI8501_WriteFile(hFileObject[nChannel], &header[nChannel], sizeof(FILE_HEADER));
		//PCI8501_ReleaseFile( hFileObject[nChannel]);
	}
