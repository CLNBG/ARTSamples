...
// 	ADBuffer[4][0] = new WORD[MAX_RAM_SIZE];
// 	ADBuffer[4][1] = new WORD[MAX_RAM_SIZE];
// 	ADBuffer[5][0] = new WORD[MAX_RAM_SIZE];
// 	ADBuffer[5][1] = new WORD[MAX_RAM_SIZE];
// 	ADBuffer[6][0] = new WORD[MAX_RAM_SIZE];
// 	ADBuffer[6][1] = new WORD[MAX_RAM_SIZE];
// 	ADBuffer[7][0] = new WORD[MAX_RAM_SIZE];
//  	ADBuffer[7][1] = new WORD[MAX_RAM_SIZE];
...
BOOL CSysApp::InitInstance()
{
	...
	/////
	int DeviceCount;
	hDevice = PCI8501_CreateDevice(0); // 创建设备对象，保存在App中，可供其他任何子窗体或子功能使用
	if(hDevice!=INVALID_HANDLE_VALUE) // 如果设备句柄有效
	{
		DeviceCount = PCI8501_GetDeviceCount(hDevice);
	...
	{
		swprintf_s(szMutex, L"PCI8501-%d", i);
	...
	if(DeviceCount!=0)
	{
		...
		m_hDevice = PCI8501_CreateDevice(m_CurrentDeviceID);
	}
	...
	gl_bRt = PCI8501_GetDDR2Length(m_hDevice,&gl_pulDDR2Length);			// DDR2的长度(单位：MB)
	if (!gl_bRt)
	{
		AfxMessageBox(L"板卡没有内存条,请插入内存条!",MB_ICONWARNING,0);
	//	return FALSE; 
	}
	gl_DDR2Lenth = gl_pulDDR2Length*1024*1024;
	if (gl_DDR2Lenth ==0)
	{
		gl_DDR2Lenth = 256*1024*1024;
	}
///////////////////////////////////////////////
...
// #endif
	...
	swprintf_s(str, L"PCI8501-%d-[%dM] " , m_CurrentDeviceID,gl_pulDDR2Length);
...
{
	// TODO: Add your command update UI handler code here
	pCmdUI->Enable(gl_bDeviceADRun==FALSE);	
}
...
{
	// TODO: Add your command handler code here
	PCI8501_ListDeviceDlg(m_hDevice); // 列表该类设备
}
...
int CSysApp::ExitInstance() 
{
	// TODO: Add your specialized code here and/or call the base class
 	...
 	{
		PCI8501_ReleaseDevice(m_hDevice);
