...
{	
	...
	case DEC_DATAMODE:	// 十进制显示
		...
		pDC->SetTextColor(RGB(0, 255, 0));
		swprintf_s(strIndex, L"%s   ", L"Index");
		for (Channel=0; Channel<MAX_CHANNEL_COUNT; Channel++)
		{			
			swprintf_s(str, L"CH%d", Channel);
			swprintf_s(strIndex, L"%s %8s     ", strIndex, str);
		}
		break;
	case HEX_DATAMODE:	// 十六进制显示
		swprintf_s(str, L"%s","  十六进制原码显示数据");
		...
		{			
			swprintf_s(str, L"CH%d", Channel);
			swprintf_s(strIndex, L"%s %16s    ", strIndex, str);
		}
	...
	for(i=rect.top; i<rect.bottom; i++)
	{
		if(i%16 == 0)
		{
			...
			case DEC_DATAMODE: // 十进制偏移码
				for(Channel=0; Channel<MAX_CHANNEL_COUNT; Channel++)
				{
					swprintf_s(str, L" %8d", ((ADBuffer[Channel][glBufferID][Index])&0xFFFF)-gl_MiddleLsb[Channel]);
			...
			case HEX_DATAMODE: // 十六进制原码
				for(Channel=0; Channel<MAX_CHANNEL_COUNT; Channel++)
				{
					swprintf_s(str, L"%5x", (ADBuffer[Channel][glBufferID][Index])&0xFFFF);
			...
			case VOLT_DATAMODE: // 电压值
				for(Channel=0; Channel<MAX_CHANNEL_COUNT; Channel++)
				{
					if (ADPara.InputRange[Channel]>= PCI8501_INPUT_0_P10000mV)
					{
						swprintf_s(str, L" %8.2f", (((ADBuffer[Channel][glBufferID][Index])&0xFFFF) - 
						(gl_MiddleLsb[Channel])) * gl_PerLsbVoltM[Channel]);
					}
					else
					{
