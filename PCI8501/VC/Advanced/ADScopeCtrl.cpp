...
CADScopeCtrl::CADScopeCtrl()
{
	...
	for( int j=0; j<MAX_CHANNEL_COUNT; j++)
	{
		...
		{	
		case PCI8501_INPUT_0_P5000mV:	// 0～5000mV
			m_dLowerLimit[j] = 0;
			m_dUpperLimit[j] = 5000;
			break;
		case PCI8501_INPUT_0_P10000mV: // 0～10000mV
			m_dLowerLimit[j] = 0;
			m_dUpperLimit[j] = 10000;
			break;
		case PCI8501_INPUT_N2500_P2500mV:	// -2500mV - +2500mV
			m_dLowerLimit[j] = -2500;
			m_dUpperLimit[j] = 2500;
			break;
...
{ 
	...
	for(int k=50; k<m_rectPlot.Width(); k+= 50) // 第隔50个像素画一条垂直的线
	{
	...
	for(int k=30; k<m_rectPlot.Height(); k+= 30) // 第隔30个像素画一条水平方向的线
	{
	...
	if(m_bAllChannel) // 显示所有通道
	{
		if(gl_bTileWave)
		{
			for(int j=1; j<m_nChannelCount; j++) // 画m_nChannelCount-1条网格线
			{
			...
			for(int j=0; j<m_nChannelCount; j++) // 画每通道的中线
			{
...
void CADScopeCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	CString str;
	CEdit* pEditM_Length=(CEdit*)(pParaCfgView->GetDlgItem(IDC_EDIT_M_Length));	// 取得IDC_EDIT_M_Length句柄
	CEdit* pEditN_Length=(CEdit*)(pParaCfgView->GetDlgItem(IDC_EDIT_N_Length));	// 取得IDC_EDIT_N_Length句柄
...
// 				//gl_NLength = gl_nSampleLenth - gl_MLength;		// 标志设置M段缓冲区大小
// 				gl_nSampleLenth = gl_NLength - gl_MLength;		// 标志设置M段缓冲区大小
// 
	...
	if(m_bDrawPoly == TRUE && point.x > m_rectPlot.left && gl_bTileWave) // 当鼠标点在边框内时
	{
		...
		if(m_bAllChannel) // 显示单通道
		{
			...
			for(int j=0; j<MAX_CHANNEL_COUNT; j++)
			{
				...
				switch(ADPara.InputRange[j])	// 电压量程
				{		
				case PCI8501_INPUT_0_P5000mV: // 0～5000mV
					m_dLowerLimit[j] = 0;
					m_dUpperLimit[j] = 5000;
				...
				case PCI8501_INPUT_0_P10000mV: // 0～10000mV
					...
					m_dUpperLimit[j] = 10000;
					break;
				case PCI8501_INPUT_N2500_P2500mV: // ±2500毫伏
					m_dLowerLimit[j] = -2500;
					m_dUpperLimit[j] = 2500;
					break;
				case PCI8501_INPUT_N5000_P5000mV: // ±5000毫伏
					m_dLowerLimit[j] = -5000;
					m_dUpperLimit[j] = 5000;
					break;
				case PCI8501_INPUT_N10000_P10000mV: // ±10000毫伏
					m_dLowerLimit[j] = -10000;
					m_dUpperLimit[j] = 10000;
					break;
				}
...
//	m_iDataLength = MAX_RAM_SIZE/MAX_CHANNEL_COUNT-glOffset;
	LONG lRAMSize = MAX_RAM_SIZE;
	
	m_iDataLength = lRAMSize/MAX_CHANNEL_COUNT-glOffset;
	if (gl_bSmlThenSize && gl_ReadSizeWords%lRAMSize!=0)
	{
		m_iDataLength = (gl_ReadSizeWords%lRAMSize)/MAX_CHANNEL_COUNT-glOffset;
	}
	...
	PerY = (int)(m_rectPlot.Height() / m_nChannelCount); // 每通道的Y宽度
  ...
  //--------------------------------------------------------
	for(Channel=0; Channel<MAX_CHANNEL_COUNT; Channel++)
	{
		...
		if(gl_bTileWave) // 平铺显示
		{
			...
			pointxy[Channel][0].y = (int)(Center) - (int)(((ADBuffer[Channel][glBufferID][glOffset]/*^0x800*/)&0xFFFF)/LsbOfPixel);
			
			for(Index=0; Index<m_iDataLength; Index++) 
			{
				pointxy[Channel][Index].x = StartX + Index;
				pointxy[Channel][Index].y = (int)(Center) - (int)(((((ADBuffer[Channel][glBufferID][Index+glOffset]/*^0x800*/)&0xFFFF) - LSB_HALF) / LsbOfPixel));
			}
		...
		else //叠加显示
		{
			...
			{
				pointxy[Channel][Index].x = StartX + Index;	
				pointxy[Channel][Index].y = (int)(Center) - (int)(((((ADBuffer[Channel][m_BufferID][Index]/*^0x800*/)&0xFFFF) - LSB_HALF) / LsbOfPixel));			
			}
		}
...
{	
	...
	{
		for(int i = 0; i<MAX_CHANNEL_COUNT; i++) // 画所有通道的点
		{
			oldPen = m_dcPlot.SelectObject(&m_polyPen[i]);
			m_dcPlot.Polyline(pointxy[i], m_iDataLength-1);			
		}
	...
	{	
		...
		//	ptOffset = m_pBuffer + m_nChannelNum*MAX_SEGMENT*MAX_OFFSET + m_BufferID*MAX_OFFSET + m_Offset; // 指针的偏移量		
			...
			pointTemp[Index].y = (int)(Center) - (int)(((((ADBuffer[m_nChannelNum][glBufferID][Index+glOffset]/*^0x800*/)&0xFFFF) - LSB_HALF - gl_MiddleLsb[m_nChannelNum]) / LsbOfPixel));
		}
...
void CADScopeCtrl::SetStatusBar()
{
	...
	fTimePixel = (float)(1000000.0/ADPara.Frequency);
	// 计算两线之间的时间差
	...
	int Center;
		...
		if (ADPara.InputRange[m_nChannelNum]==PCI8501_INPUT_0_P10000mV||ADPara.InputRange[m_nChannelNum]==PCI8501_INPUT_0_P5000mV)

		{
			 Center = m_rectPlot.Height()/*/2*/ + m_rectPlot.top;
		}
		else
		{
              Center = m_rectPlot.Height()/2 + m_rectPlot.top;
...
int CADScopeCtrl::FindTrigger(int nChannel, int nValue) // 防止波形抖动
{
	...
	{
		int Temp = ADBuffer[nChannel][m_BufferID][Index]&0xFFFF;
		if(Temp<(ADBuffer[nChannel][m_BufferID][Index+1]&0xFFFF))
		{
...
void CADScopeCtrl::DrawAllChannelText(CDC *pDC)
{
	...
	for(int i=0; i<MAX_CHANNEL_COUNT; i++)
	{
		pDC->SetTextColor(m_clPen[i]); // 设置文字的颜色
		str.Format (L"%.*lf V", m_nYDecimals, m_dUpperLimit[i]/1000.0); // 正电压值
		pDC->TextOut (m_rectPlot.left-4, (int)(m_rectPlot.top+hight*nCount+8), str); 
		
		pDC->SetTextAlign (TA_RIGHT|TA_BASELINE);
		str.Format (L"%.*lf V", m_nYDecimals, m_dLowerLimit[i]/1000.0); // 负电压值
