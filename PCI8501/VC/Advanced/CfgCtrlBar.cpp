...
BOOL CCfgCtrlBar::Create(CWnd* pParentWnd, CSize sizeDefault, UINT nID, DWORD dwStyle) 
{
    ...
    if (!CWnd::Create(wndclass, L"ÅäÖÃÓë·ÖÎöÌõ", dwStyle, CRect(0,0,0,0),
        pParentWnd, nID))
        return FALSE;
...
CSize CCfgCtrlBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
    ...
    if (bHorz)
        return CSize(nHorzDockBarWidth, m_sizeHorz.cy);	// 660);//m_sizeHorz.cy);
    else
        return CSize(m_sizeVert.cx,nVertDockBarHeight);	// 660);//nVertDockBarHeight);
}
