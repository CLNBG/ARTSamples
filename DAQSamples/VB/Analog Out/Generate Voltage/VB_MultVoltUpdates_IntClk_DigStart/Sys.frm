...
Private Sub Command_button_Click()
    ...
    If ArtDAQ_CfgSampClkTiming(taskHandle, 0, 1000#, ArtDAQ_Val_Rising, ArtDAQ_Val_FiniteSamps, 4000) <> 0 Then
        MsgBox "配置时钟参数失败!"
        ArtDAQ_ClearTask (taskHandle)
        Exit Sub
    End If
    ...
    If ArtDAQ_CfgDigEdgeStartTrig(taskHandle, str(0), ArtDAQ_Val_Rising) <> 0 Then
        MsgBox "配置数字边沿开始触发参数失败!"
        ArtDAQ_ClearTask (taskHandle)
        Exit Sub
    End If
    
    For index = 0 To 3999
        data(index) = (5# * index) / 4000#
    Next
    
    If ArtDAQ_WriteAnalogF64(taskHandle, 4000, 0, 10#, ArtDAQ_Val_GroupByChannel, data(0), written, 0) <> 0 Then
        MsgBox "写入AO失败!"
        ArtDAQ_ClearTask (taskHandle)
        Exit Sub
    End If
...
Private Sub Form_Load()
    Label1.Caption = "本例程演示的是采用内部时钟、数字边沿开始触发AO有限输出的简易流程"
    bRun = False
End Sub
