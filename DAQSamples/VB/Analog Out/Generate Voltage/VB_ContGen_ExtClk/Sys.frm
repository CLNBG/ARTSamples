...
Private Sub Command_button_Click()
    ...
    For index = 1 To 1000
        data(index) = 9.95 * Sin(index * 2# * "6.28318531" / 1000#)
    Next
    
    If ArtDAQ_WriteAnalogF64(taskHandle, 1000, 0, 10#, ArtDAQ_Val_GroupByChannel, data(1), 0, 0) <> 0 Then
        MsgBox "写入AO失败!"
        ArtDAQ_ClearTask (taskHandle)
        Exit Sub
    End If
...
Private Sub Form_Load()
    Label1.Caption = "本例程演示的是采用外部时钟进行AO连续输出的简易流程"
    bRun = False
End Sub
