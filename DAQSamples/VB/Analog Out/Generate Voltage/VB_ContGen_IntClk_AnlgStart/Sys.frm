...
Private Sub Command_button_Click()
    ...
    str = StrConv("/Dev1/APFI" & vbNullChar, vbFromUnicode)
    If ArtDAQ_CfgAnlgEdgeStartTrig(taskHandle, str(0), ArtDAQ_Val_Rising, 1#) <> 0 Then
        MsgBox "配置模拟边沿开始触发参数失败!"
        ArtDAQ_ClearTask (taskHandle)
        Exit Sub
    End If
...
Private Sub Form_Load()
    Label1.Caption = "本例程演示的是采用内部时钟、模拟边沿开始触发AO连续输出的简易流程"
    bRun = False
End Sub
