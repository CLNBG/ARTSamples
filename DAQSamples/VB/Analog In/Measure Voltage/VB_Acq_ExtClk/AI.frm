...
Private Sub Command1_Click()
    ...
    str = StrConv("/Dev1/PFI2" & vbNullChar, vbFromUnicode)
    If ArtDAQ_CfgSampClkTiming(taskHandle, 0, 1000#, ArtDAQ_Val_Rising, ArtDAQ_Val_FiniteSamps, 1000) <> 0 Then     'ADC转换时钟
        MsgBox "配置采集参数失败!"
        ArtDAQ_ClearTask (taskHandle)
        Exit Sub
    End If
    If ArtDAQ_SetAIConvClk(taskHandle, str(0), ArtDAQ_Val_Rising) <> 0 Then
            MsgBox "配置采集参数失败!"
        ArtDAQ_ClearTask (taskHandle)
        Exit Sub
    End If
    ...
    If ArtDAQ_ReadAnalogF64(taskHandle, -1, 10#, ArtDAQ_Val_GroupByChannel, data(0), 1000, read, 0) <> 0 Then
        MsgBox "数据读取失败!"
