...
Private Sub Command_button_Click()
    ...
    ret = ArtDAQ_CreateCIFreqChan(taskHandle, str(0), 0, 0.232831, 50000000#, ArtDAQ_Val_Hz, ArtDAQ_Val_Rising, ArtDAQ_Val_LargeRng2Ctr, 0.001, 10, 0)
    If ret <> 0 Then
        MsgBox "创建计数器失败!"
    ...
    Do While bRun = True
        If ArtDAQ_ReadCounterF64(taskHandle, 1000, 10#, data(0), 1000, bytesPerSamp, 0) <> 0 Then
            MsgBox "读取计数失败!"
            ArtDAQ_ClearTask (taskHandle)
            Exit Sub
        End If
        Label2.Caption = "Acquired " & CStr(bytesPerSamp)
        DoEvents
    Loop
...
Private Sub Form_Load()
    Label1.Caption = "本例程演示的是数字边沿开始触发生成连续脉冲的简易流程"
    bRun = False
End Sub
