...
Private Sub Command_button_Click()
    ...
    ret = ArtDAQ_CreateCIPeriodChan(taskHandle, str(0), 0, 0.000000002, 4.29496, ArtDAQ_Val_Seconds, ArtDAQ_Val_Rising, ArtDAQ_Val_LargeRng2Ctr, 0.001, 10, 0)
    If ret <> 0 Then
        MsgBox "创建测量周期通道失败!"
    ...
    Do While bRun = True
        If ArtDAQ_ReadCounterF64(taskHandle, 1000, 10#, data(0), 1000, sampsRead, 0) <> 0 Then
            ...
            Exit Sub
        End If
        Label2.Caption = "Acquired " & CStr(sampsRead) & " Hz"
        DoEvents
    Loop
...
Private Sub Form_Load()
    Label1.Caption = "本例程演示的是测量周期的简易流程"
End Sub
