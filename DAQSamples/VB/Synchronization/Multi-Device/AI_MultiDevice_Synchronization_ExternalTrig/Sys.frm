...
Private Sub Command_button_Click()
    ...
    If ArtDAQ_CfgDigEdgeStartTrig(masterTaskHandle, 0, ArtDAQ_Val_Rising) <> 0 Then
        MsgBox "配置主卡数字边沿开始触发参数失败!"
        ArtDAQ_ClearTask (masterTaskHandle)
        Exit Sub
    End If
    str = StrConv("/Dev2/PFI1" & vbNullChar, vbFromUnicode)
    If ArtDAQ_CfgDigEdgeStartTrig(slaveTaskHandle, 0, ArtDAQ_Val_Rising) <> 0 Then
        MsgBox "配置从卡数字边沿开始触发参数失败!"
        ArtDAQ_ClearTask (slaveTaskHandle)
        Exit Sub
    End If
...
Private Sub Form_Load()
    Label1.Caption = "本例程演示的是主从卡同步采用内部时钟、数字边沿开始触发进行采集的简易流程"
    bRun = False
End Sub
