...
Private Sub Command_button_Click()
    ...
    If mode = 0 Then
        ...
        If ArtDAQ_SetAIConvClk(masterTaskHandle, str(0), ArtDAQ_Val_Rising) <> 0 Then
            MsgBox "配置主卡转换时钟参数失败!"
            ArtDAQ_ClearTask (masterTaskHandle)
            Exit Sub
        End If
    ...
    Else
        '主从卡使用外部时钟作为采样时钟
        str = StrConv("/Dev1/PFI2" & vbNullChar, vbFromUnicode)
        If ArtDAQ_CfgSampClkTiming(masterTaskHandle, str(0), 10000#, ArtDAQ_Val_Rising, ArtDAQ_Val_ContSamps, 10000) <> 0 Then
            MsgBox "配置主卡外部采样时钟参数失败!"
            ArtDAQ_ClearTask (masterTaskHandle)
            Exit Sub
        End If
        str = StrConv("/Dev2/PFI2" & vbNullChar, vbFromUnicode)
