...
begin
  ...
  ret := ArtDAQ_CreateCIFreqChan(taskHandle, pchar(strChannelName), '', 0.023284, 50000000.000, ArtDAQ_Val_Hz, ArtDAQ_Val_Rising, ArtDAQ_Val_LowFreq1Ctr, 0.001, 4, '');
  if(ret < 0) then
  begin
    Writeln('创建CTR0失败!');
    goto ExitSys;
  ...
  if(ret < 0) then
    ...
    begin
      Writeln(Format('Frequency: %.9f Hz',[data[0]]));
    end;
