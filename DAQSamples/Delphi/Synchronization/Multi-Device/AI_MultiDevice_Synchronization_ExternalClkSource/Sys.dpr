...
Var
  ...
  strMasterExtSource:string;
  strSlaveExtSource:string;
...
begin
  ...
  mode := 0;
  strMasterExtSource := '/' + strMasterCardName + '/PFI2';
  strSlaveExtSource := '/' + strSlaveCardName + '/PFI2';
  if(mode = 0) then
  begin
    ...
    // 主从卡使用外部时钟作为转换时钟
    ret := ArtDAQ_SetAIConvClk(masterTaskHandle, pchar(strMasterExtSource), ArtDAQ_Val_Rising);
    if(ret < 0) then
    begin
      Writeln('主卡配置转换时钟失败!');
      goto ExitSys;
    end;
    ret := ArtDAQ_SetAIConvClk(slaveTaskHandle, pchar(strSlaveExtSource), ArtDAQ_Val_Rising);
    if(ret < 0) then
    begin
      Writeln('从卡配置转换时钟失败!');
      goto ExitSys;
  ...
  begin
    ret := ArtDAQ_CfgSampClkTiming(masterTaskHandle, pchar(strMasterExtSource), 10000.0, ArtDAQ_Val_Rising, ArtDAQ_Val_ContSamps, 10000);
    if(ret < 0) then
    begin
      Writeln('主卡配置采样时钟失败!');
      goto ExitSys;
    end;
    ret := ArtDAQ_CfgSampClkTiming(slaveTaskHandle, pchar(strSlaveExtSource), 10000.0, ArtDAQ_Val_Rising, ArtDAQ_Val_ContSamps, 10000);
    if(ret < 0) then
    begin
      Writeln('从卡配置采样时钟失败!');
      goto ExitSys;
