...
begin
  ...
  // USB2861不支持此功能
  ret := ArtDAQ_CfgDigEdgeRefTrig(taskHandle, pchar(strTrigSource), ArtDAQ_Val_Rising, 100);
  if(ret < 0) then
  begin
    Writeln('配置触发失败!');
    goto ExitSys;
  ...
  //*********************************************/
	// ArtDAQ Read Code
	//*********************************************/
  ret := ArtDAQ_ReadAnalogF64(taskHandle, 1000, 1.0, ArtDAQ_Val_GroupByChannel, @data, 1000, @read, nil);
  if(ret < 0) then
  begin
    Writeln('读数失败!');
    goto ExitSys;
