...
namespace Sys
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            error = ArtDAQ.ArtDAQ_CfgSampClkTiming(taskHandle, "", 10000.0, ArtDAQ.ArtDAQ_Val_Rising, ArtDAQ.ArtDAQ_Val_ContSamps, 1000);
            GetErrorString(error);
            if (error < 0)
            {
                goto Exit;
            ...
            //step4:Configure Extern Clock
            strSource = "/" + strCardName + "/PFI2";
            error = ArtDAQ.ArtDAQ_SetAIConvClk(taskHandle, strSource, ArtDAQ.ArtDAQ_Val_Rising);
            GetErrorString(error);
            if (error < 0)
            {
                goto Exit;
