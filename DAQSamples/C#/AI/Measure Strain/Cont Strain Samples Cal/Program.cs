...
namespace Sys
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //step2: CreateChannel
            error = ArtDAQ.ArtDAQ_CreateAIStrainGageChan(taskHandle, strChannelName, "", -0.001, 0.001, ArtDAQ.ArtDAQ_Val_Strain, ArtDAQ.ArtDAQ_Val_FullBridgeI, ArtDAQ.ArtDAQ_Val_Internal, 2.50, 2.0, 0.0, 120.0, 0.285, 0.0, "");
            ...
            error = ArtDAQ.ArtDAQ_CfgSampClkTiming(taskHandle, "", 10.0, ArtDAQ.ArtDAQ_Val_Rising, ArtDAQ.ArtDAQ_Val_FiniteSamps, 1000);
            GetErrorString(error);
            if (error < 0)
            {
                goto Exit;
            }
            error = ArtDAQ.ArtDAQ_PerformBridgeOffsetNullingCal(taskHandle, "");
            GetErrorString(error);
            if (error < 0)
            {
                goto Exit;
            ...
            error = ArtDAQ.ArtDAQ_PerformStrainShuntCal(taskHandle, "", 100000, 0, 0);
            GetErrorString(error);
            if (error < 0)
            {
                goto Exit;
            ...
            while (_kbhit() == 0)
            {
                error = ArtDAQ.ArtDAQ_ReadAnalogF64(taskHandle, -1, 10.0, ArtDAQ.ArtDAQ_Val_GroupByScanNumber, data, 1000, out read, (IntPtr)(0));
