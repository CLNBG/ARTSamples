...
namespace Sys
{
    class Program
    {
        ...
        public static Int32 DoneCallback(IntPtr taskHandle, Int32 status, IntPtr callbackData)
        {
            Int32 error = 0;
        ...
        Exit:
            if (error < 0)
            {
                ArtDAQ.ArtDAQ_GetExtendedErrorInfo(errBuff, 2048);
                ArtDAQ.ArtDAQ_ClearTask(taskHandle);
                Console.WriteLine("ArtDAQ Error: {0:s}", errBuff);
            }
            return 0;
        }
        ...
        static void Main(string[] args)
        {
            ...
            error = ArtDAQ.ArtDAQ_CfgSampClkTiming(taskHandle, "/Dev4/PFI0", 200, ArtDAQ.ArtDAQ_Val_Rising, ArtDAQ.ArtDAQ_Val_ContSamps, 1000);
            GetErrorString(error);
            if (error < 0)
            {
                goto Exit;
