...
int main(void)
{
	...
	float64	AOdata[1000];
	float64	phase=0.0;
	char        strAIChannelName[100]={'\0'};
	char        strAOChannelName[100]={'\0'};
	char        strAIStartTrigger[100]={'\0'};
	char        strAOStartTrigger[100]={'\0'};

	/*********************************************/
	// ArtDAQ Configure Code
	/*********************************************/
	printf("Please enter AI channal number,such as Dev1/ai0,Dev1 is the name identified in the DMC:");
    scanf("%s", strAIChannelName);
	printf("Please enter AO channal number,such as Dev1/ao0,Dev1 is the name identified in the DMC:");
    scanf("%s", strAOChannelName);
	printf("Please enter AI digial start trigger source name,such as Dev1/PFI1,Dev1 is the name identified in the DMC:");
    scanf("%s", strAIStartTrigger);
	printf("Please enter AO digial start trigger source name,such as Dev1/PFI1,Dev1 is the name identified in the DMC:");
    scanf("%s", strAOStartTrigger);
	// All
	ArtDAQErrChk (ArtDAQ_CreateTask("",&AItaskHandle));
	ArtDAQErrChk (ArtDAQ_CreateAIVoltageChan(AItaskHandle,strAIChannelName,"",ArtDAQ_Val_Cfg_Default,-10.0,10.0,ArtDAQ_Val_Volts,NULL));
	ArtDAQErrChk (ArtDAQ_CfgSampClkTiming(AItaskHandle,"",10000.0,ArtDAQ_Val_Rising,ArtDAQ_Val_ContSamps,1000));
	ArtDAQErrChk (ArtDAQ_CfgDigEdgeStartTrig(AItaskHandle,strAIStartTrigger,ArtDAQ_Val_Rising));
	ArtDAQErrChk (ArtDAQ_CreateTask("",&AOtaskHandle));
	ArtDAQErrChk (ArtDAQ_CreateAOVoltageChan(AOtaskHandle,strAOChannelName,"",-10.0,10.0,ArtDAQ_Val_Volts,NULL));
	ArtDAQErrChk (ArtDAQ_CfgSampClkTiming(AOtaskHandle,"",5000.0,ArtDAQ_Val_Rising,ArtDAQ_Val_ContSamps,1000));
	ArtDAQErrChk (ArtDAQ_CfgDigEdgeStartTrig(AOtaskHandle,strAOStartTrigger,ArtDAQ_Val_Rising));
