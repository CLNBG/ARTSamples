...
int main(void)
{
	int32       error=0;
    char        errBuff[2048]={'\0'};
    int         mode;   // 0： 异步卡多卡同步模式  1： 同步卡同步模式
    ...
    if (mode == 0)
    {
        ...
        ArtDAQErrChk (ArtDAQ_ExportSignal(masterTaskHandle, ArtDAQ_Val_AIConvertClock, "/Dev1/PFI3"));
        ...
        ArtDAQErrChk (ArtDAQ_SetAIConvClk(slaveTaskHandle,"/Dev2/PFI2",ArtDAQ_Val_Rising));
    }
    else
    {
        // Master device use internal clock 
        ...
        ArtDAQErrChk (ArtDAQ_ExportSignal(masterTaskHandle, ArtDAQ_Val_SampleClock, "/Dev1/PFI3"));
