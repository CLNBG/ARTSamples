...
*		per Channel is set the same for both devices. Set the refrence 
*		clock to external 10M clock for both devices.
...
int main(void)
{
    ...
    ArtDAQErrChk (ArtDAQ_SetRefClkSrc(masterTaskHandle,"PXI_Clk10"));
    // Slave device use external clock
    ArtDAQErrChk (ArtDAQ_SetRefClkSrc(slaveTaskHandle,"PXI_Clk10"));

    // 开始触发
    ArtDAQErrChk (ArtDAQ_ExportSignal(masterTaskHandle, ArtDAQ_Val_StartTrigger, "PXI_Trig0"));
    ArtDAQErrChk (ArtDAQ_CfgDigEdgeStartTrig(slaveTaskHandle,"PXI_Trig0",ArtDAQ_Val_Rising));
