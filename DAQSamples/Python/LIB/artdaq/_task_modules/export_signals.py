...
from artdaq.constants import (Polarity, ExportAction, ToggleIdleState, PulseWidthMode)
...
class ExportSignals(object):
    ...
    def export_ctrComparsionEvent(self, output_terminal, output_behavior=ExportAction.PULSE,
                            pulse_polarity=Polarity.ACTIVE_HIGH, toggle_idlestate=ToggleIdleState.HIGH,
                           pulse_widthmode= PulseWidthMode.PRESET):

        cfunc = lib_importer.windll.ArtDAQ_ExportPositionComparsionEvent
        if cfunc.argtypes is None:
            with cfunc.arglock:
                 if cfunc.argtypes is None:
                     cfunc.argtypes = [
        ...
        error_code = cfunc(
                self._handle, output_terminal, output_behavior.value, pulse_polarity.value, toggle_idlestate.value, pulse_widthmode.value)
        check_for_error(error_code)
