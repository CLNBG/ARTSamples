...
BOOL CFileOpenDlg::ReadFileHead(CString strFilepath, int Channel)
{
	...
	if (!file.Open(strFilepath, PCI8510_modeRead)) // 打开文件
		return FALSE;
	...
	{
		AfxMessageBox(_T("此文件并非PCI8510板卡配套文件，请重新选择!"));
		return FALSE;
	}
...
void CFileOpenDlg::InintFilePath(CString strPath)
{
	...
	if (!file.Open(strPath, PCI8510_modeRead)) // 打开文件
	{
	...
	if (!file.Open(strWhole, PCI8510_modeRead)) // 打开文件strWhole
	{
