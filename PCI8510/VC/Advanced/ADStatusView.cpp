...
void CADStatusView::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_Analyze, m_Check_Analyze);
...
BEGIN_MESSAGE_MAP(CADStatusView, CFormView)
	...
	ON_BN_CLICKED(IDC_CHECK_Analyze, OnCHECKAnalyze)
...
void CADStatusView::OnInitialUpdate() 
{
	...
	if (!m_Check_Analyze.GetCheck())
	{
		m_Edit_BottomTringgerVolt.EnableWindow(FALSE);
		m_Edit_TopTringgerVolt.EnableWindow(FALSE);
		m_Combo_ScreenVolume.EnableWindow(FALSE);
		m_Combo_MoveVolt.EnableWindow(FALSE);
	}
...
void CADStatusView::OnCHECKAnalyze()
{
	if (m_Check_Analyze.GetCheck())
	{
		AfxMessageBox(_T("请在单通道显示时使用精度分析"));
		m_Edit_BottomTringgerVolt.EnableWindow(TRUE);
		m_Edit_TopTringgerVolt.EnableWindow(TRUE);
		m_Combo_ScreenVolume.EnableWindow(TRUE);
		m_Combo_MoveVolt.EnableWindow(TRUE);
	}
	else
	{
		m_Edit_BottomTringgerVolt.EnableWindow(FALSE);
...
void CADStatusView::OnSelchangeComboScreenVolume() 
{
	...
	gl_Screen = _tcstol(strScreenVolume, NULL, 10);
	...
	{
		if (ADPara.InputRange > 1)
		{
			
			pWaveView->m_ADScopeCtrl.SetRange(-gl_ScreenVolume/2.0, gl_ScreenVolume/2.0, nCH);
		}
		else
		{
			
			pWaveView->m_ADScopeCtrl.SetRange(0, gl_ScreenVolume, nCH);
		}
	}
}
...
void CADStatusView::AnalyzeAccurate() // 精度分析
{
	...
	m_TopTriggerLsb = (int)(gl_TopTriggerVolt*(AD_LSB_COUNT/gl_VoltRange)); // 上下跳动的值(电压值--->码值)
	m_BottomTriggerLsb = (int)(gl_BottomTriggerVolt*(AD_LSB_COUNT/gl_VoltRange)); // 上下跳动的值(电压值--->码值)
