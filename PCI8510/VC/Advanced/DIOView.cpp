...
void CDIOView::SetSwitchStatus(void)
{
	if (!bCreateDevice)
	 ...
	 BYTE DISts[8];
	// 读入各路开关量状态
	if (!PCI8510_GetDeviceDI(hDevice, DISts))
	{
...
void CDIOView::SetSwitchOut(void)
{
	...
	Flag = true;
	// 输出各路开关量状态
	BOOL  bRet = PCI8510_SetDeviceDO(hDevice, DOSts); 
...
//	hDevice = PXI8501_CreateDevice(DeviceID);
	...
	BYTE DOSts[8];
	//PXI8501_RetDeviceDO( hDevice, DOSts);
...
// 	if (!PXI8501_ReleaseDevice(hDevice))
// 		AfxMessageBox(_T("设备释放失败"));
 	CFormView::OnDestroy();	
}
