...
int CADFrm::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	PCI8510_LoadParaAD(NULL, &ADPara);  // 读入系统参数
...
void CADFrm::OnMoveVolt() // 平移电压的函数
{
	...
	if (gl_bProcessMoveVolt) // 开始求中间值
	{
		for (Channel=0; Channel<MAX_CHANNEL_COUNT; Channel++)
		{
					...
					{
						DataAnalyze[Channel][Index] = ((ADBuffer[gl_nDrawIndex][Index * gl_nChannelCount + Channel])&MASK_MSB); 
					}
		...
		for (Channel=0; Channel<MAX_CHANNEL_COUNT; Channel++) // 每通道的数据各自进行排序
		{
			...
			case SHOW_MODE_DEC: // 十进制原码
				swprintf_s(str, _T("CH%d %d Dec"), Channel, m_DataMiddle);
				break;
			...
			case SHOW_MODE_VOLT: // 电压值
				...
				swprintf_s(str, _T("CH%d %5.2fmV"), Channel, (m_DataMiddle - AD_LSB_HALF)*gl_PerLsbVolt[Channel]);
				break;
			}
...
void CADFrm::OnDestroy() 
{
	...
	PCI8510_SaveParaAD(NULL, &ADPara);
