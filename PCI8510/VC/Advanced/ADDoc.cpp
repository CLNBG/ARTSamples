...
{	
	...
	gl_TopTriggerVolt = (int)gl_VoltRange; // 置触发电平的初值为其最大量程
	gl_TopTriggerVolt = 0;
...
void CADDoc::StopDeviceAD() 
{
	...
	PCI8510_ReleaseSystemEvent(gl_hEvent); // 释放消息
...
void CADDoc::OnDataFileNew() 
{
	...
	if (!gl_bCreateFile)
	{
		...
		{
			AfxMessageBox(_T("释放文件对象失败..."), MB_ICONSTOP, 0);
		}
	}
	...
	//ULONGLONG DiskFreeBytes = PCI8510_GetDiskFreeBytes(strNewFileName.Left(3));  // 取得文件所在磁盘可用空间
