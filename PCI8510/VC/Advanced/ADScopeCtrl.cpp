		...
		if (m_bAllChannel) 
		{
			for (int Channel=0; Channel<MAX_AD_CHANNELS; Channel++) //判断鼠标双击位置
			{
				...
				{	
					...
					gl_ScreenVolume = (int)(m_dUpperLimit[Channel]-m_dLowerLimit[Channel]);
			...
			{
				m_dLowerLimit[m_nChannelNum] = 0;
				m_dUpperLimit[m_nChannelNum] = gl_ScreenVolume;	
			}		
		}
...
{	
	UpdateChannelCount();
	float m_ScreenVolume = (float)gl_VoltRange; // 多通道显示时，屏幕是量程是和最大电压值一致的
	float LsbOfPixel;
	
	LsbOfPixel = (float)(((gl_ScreenVolume/gl_VoltRange)*AD_LSB_COUNT)/(PerY)); // 每像素对应的码值 
	...
	if (gl_bTileWave) // 多通道平铺显示
	{
		...
		{			
			for (Index=0; Index<=(m_nPlotWidth-m_Offset); Index++) // 初始化1024个点(创建时，位图的大小) 
			{
				ptOffset = (PLONG)&ADBuffer[gl_nDrawIndex][(m_Offset +Index) * m_nChannelCount]; // 指针的偏移量
				pointxy[TChannel][Index].x = StartX + Index;
				
				if (!gl_bProcessMoveVolt)
				{
					DataY = (int)(((((ptOffset[TChannel])&MASK_MSB) - gl_MiddleLsb[Channel]) / LsbOfPixel)); // 将原始码数据减去平移电压码值和半幅偏移码值
				}
				else
				{
	...
	else // 多通道叠加显示
	{
		for (Channel=0; Channel<MAX_AD_CHANNELS; Channel++)
		{			
			LsbOfPixel = (float)(((gl_ScreenVolume/gl_VoltRange)*AD_LSB_COUNT)/(m_rectPlot.Height())); // 每像素对应的码值    
			int StartX = m_rectPlot.left;
			Center = (int)(m_rectPlot.Height() / 2) + m_rectPlot.top;
			
			for (int Index=0; Index<=(m_nPlotWidth-m_Offset); Index++)	
			{
				ptOffset = (PLONG)&ADBuffer[gl_nDrawIndex][(m_Offset +Index) * m_nChannelCount]; // 指针的偏移量
				pointxy[TChannel][Index].x = StartX + Index;	
				DataY = (int)(((((ptOffset[TChannel])&MASK_MSB) - AD_LSB_HALF) / LsbOfPixel));
...
{	
	...
	{	
		...
		for (int Index=0; Index<=(m_nPlotWidth-m_Offset); Index++)	
		{
			...
			if (!gl_bProcessMoveVolt)
			{
				DataY = (int)(((((ptOffset[m_nChannel])&MASK_MSB) - gl_MiddleLsb[m_nChannelNum]) / LsbOfPixel));
			//	DataY = (int)(((((ptOffset[Index * m_nChannelCount + m_nChannel])&MASK_MSB) - gl_MiddleLsb[m_nChannelNum]) / LsbOfPixel));
			}
			else
			{		
				DataY = (int)(((((ptOffset[m_nChannel])&MASK_MSB) - AD_LSB_HALF) / LsbOfPixel));
			}
...
void CADScopeCtrl::SetStatusBar()
{
	...
	{
		Center = m_rectPlot.Height() + m_rectPlot.top;
	}
...
void CADScopeCtrl::DrawSingleCHText(CDC* pDC, int nChannelNum)
{
	...
	else
	{
		...
		strInfo.Format (_T("%.*lf mV"), 0, m_dUpperLimit[0]); // 正电压值
		pDC->TextOut(m_rectPlot.left-4, m_rectPlot.top, strInfo); 
		pDC->SetTextAlign (TA_RIGHT|TA_BASELINE);
		strInfo.Format (_T("%.*lf mV"), 0, m_dLowerLimit[0]); // 负电压值
		pDC->TextOut(m_rectPlot.left-4, m_rectPlot.top+m_nPlotHeight, strInfo);
		strInfo = "CH";
		pDC->TextOut(m_rectPlot.left-4, m_rectPlot.top+m_nPlotHeight/2+5, strInfo);      
	}
...
// 	
//  	strInfo.Format (_T("%.*lf mV"), 0, gl_Screen/2.0); // 正电压值
// 	pDC->TextOut (m_rectPlot.left-4, m_rectPlot.top, strInfo); 
// 	pDC->SetTextAlign (TA_RIGHT|TA_BASELINE);
// 	strInfo.Format (_T("%.*lf mV"), 0, -gl_Screen/2.0); // 负电压值
// 	
// 	pDC->TextOut (m_rectPlot.left-4, m_rectPlot.top+m_nPlotHeight, strInfo);
// 	
