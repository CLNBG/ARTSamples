...
// 	pList->AddString(str); // 触发位置
	...
	pCardTypeStatic->SetWindowText(pDoc->m_File0.GetFilePath()); // 文件路径名
	...
	str.Format(_T("数据回放:%s"), pDoc->m_File0.GetFileName());
	pDoc->SetTitle(str); 
...
void CADHistDigitView::SetListView(void)
{
	...
	{
		swprintf_s(strList, _T("%d"), pDoc->m_Offset+Row);
		...
		for (int Channel=0; Channel<MAX_CHANNEL_COUNT; Channel++)
		{
			...
			{
			case 1: // 十进制显示
				if (pDoc->m_Header.VoltBottomRange!=0)
				{
					if (!gl_bHistProcessMoveVolt)
					{
						swprintf_s(strList, _T("%d"), ((pDoc->m_ADBuffer[index]) & MASK_MSB) - gl_HistMiddleLsb[Channel]);
					}
					else
					{
						swprintf_s(strList, _T("%d"), ((pDoc->m_ADBuffer[index]) & MASK_MSB));
					}		
				}
				else
				{
			...
			case 2: // 十六进制显示
				...
				swprintf_s(strList, _T("%6x"), ADLsb & MASK_MSB);
				break;
				
			case 3:
				if (pDoc->m_Header.VoltBottomRange[0]!=0)
				{
				...
				swprintf_s(strList, _T("%8.2f"), fADVolt);	
				break;					
			}
