...
void CADHistScope::InvalidateCtrl()
{
 ...
 // ADPara = pDoc->m_Header.ADPara;
	...
	for (Channel=0; Channel<MAX_CHANNEL_COUNT; Channel++)
	{		
		m_dLowerLimit[Channel] = pDoc->m_Header.VoltBottomRange[Channel];
		m_dUpperLimit[Channel] = pDoc->m_Header.VoltTopRange[Channel];
	}
...
{	
	...
	if (gl_bTileWave) // 多通道平铺显示
	{
		for (Channel=0; Channel<m_nChannelCount; Channel++)
		{
			...
			if (pHistDoc->m_Header.VoltBottomRange!=0)
			{
				...
				{
					if ((Offset + Index) * m_nChannelCount + Channel +m_Offset >= HALF_SIZE_WORDS)
						break;
...
{	
	...
	int nDrawCount = HALF_SIZE_WORDS/m_nChannelCount; 
	...
	{	
		...
		if (pHistDoc->m_Header.VoltBottomRange!=0)
		{
			for (int Index=0; Index<=m_nPlotWidth; Index++)	
			{
				...
				DataY = (int)(((((ptOffset[Index])&MASK_MSB) - AD_LSB_HALF - gl_HistMiddleLsb[m_nChannelNum]) / LsbOfPixel));
