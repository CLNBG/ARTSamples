Module PCI8510
    ...
    Declare Function PCI8510_CreateDevice Lib "PCI8510_64" (ByVal DeviceLgcID As Int32) As IntPtr      ' 用逻辑号创建设备对象
    Declare Function PCI8510_GetDeviceCount Lib "PCI8510_64" (ByVal hDevice As IntPtr) As Int32  ' 取得设备总台数
    Declare Function PCI8510_GetDeviceCurrentID Lib "PCI8510_64" (ByVal hDevice As IntPtr, ByRef DeviceLgcID As Int32, ByRef DevicePhysID As Int32) As Boolean
    Declare Function PCI8510_ReleaseDevice Lib "PCI8510_64" (ByVal hDevice As IntPtr) As Boolean  ' 仅释放设备对象
    ...
    ' AD程序查询方式函数
    Declare Function PCI8510_InitDeviceProAD Lib "PCI8510_64" ( _
    ...
    Declare Function PCI8510_StartDeviceProAD Lib "PCI8510_64" ( _
         ByVal hDevice As IntPtr) As Boolean         ' 在初始化之后，启动设备' 设备对象句柄,它由CreateDevice函数创建

    Declare Function PCI8510_ReadDeviceProAD_Npt Lib "PCI8510_64" ( _
            ByVal hDevice As IntPtr, _
    ...
    Declare Function PCI8510_GetDevStatusProAD Lib "PCI8510_64" ( _
            ByVal hDevice As IntPtr, _
            ByRef pADStatus As PCI8510_STATUS_AD) As Boolean  ' 在AD采样过程中取得设备的各种状态,返回值表示函数是否成功' 设备句柄,它由CreateDevice函数创建' AD的各种信息结构体

    Declare Function PCI8510_ReadDeviceProAD_Half Lib "PCI8510_64" ( _
            ByVal hDevice As IntPtr, _
    ...
    Declare Function PCI8510_StopDeviceProAD Lib "PCI8510_64" ( _
         ByVal hDevice As IntPtr) As Boolean         ' 在启动设备之后，暂停设备' 设备对象句柄,它由CreateDevice函数创建

    Declare Function PCI8510_ReleaseDeviceProAD Lib "PCI8510_64" ( _
         ByVal hDevice As IntPtr) As Boolean         ' 关闭AD设备,禁止传输,且释放资源' 设备句柄,它由CreateDevice函数创建

    '##################### ' AD直接内存(DMA)方式函数) ###########################
    Declare Function PCI8510_InitDeviceDmaAD Lib "PCI8510_64" ( _
         ByVal hDevice As IntPtr, _
    ...
    Declare Function PCI8510_StartDeviceDmaAD Lib "PCI8510_64" ( _
         ByVal hDevice As IntPtr) As Boolean
    ' 在初始化之后，启动设备 ' 设备对象句柄,它由CreateDevice函数创建
    Declare Function PCI8510_GetDevStatusDmaAD Lib "PCI8510_64" ( _
         ByVal hDevice As IntPtr, _
         ByRef pDMAStatus As PCI8510_STATUS_DMA) As Boolean ' 在AD采样过程中取得DMA的有关状态,返回值表示函数是否成功 ' 设备句柄,它由CreateDevice函数创建 ' 获得的DMA工作状态

    Declare Function PCI8510_SetDevStatusDmaAD Lib "PCI8510_64" ( _
            ByVal hDevice As IntPtr, _
            ByVal iClrBufferID As Int32) As Boolean         ' 在AD采样过程中设置DMA的有关状态,返回值表示函数是否成功' 设备句柄,它由CreateDevice函数创建' 要清除的缓冲区ID,将其置为0

    Declare Function PCI8510_StopDeviceDmaAD Lib "PCI8510_64" ( _
         ByVal hDevice As IntPtr) As Boolean        ' 在启动设备之后，暂停设备' 设备对象句柄,它由CreateDevice函数创建

    Declare Function PCI8510_ReleaseDeviceDmaAD Lib "PCI8510_64" ( _
         ByVal hDevice As IntPtr) As Boolean      ' 关闭AD设备,禁止传输,且释放资源' 设备句柄,它由CreateDevice函数创建

    Declare Function PCI8510_ADCalibration Lib "PCI8510_64" ( _
         ByVal hDevice As IntPtr) As Boolean  ' 设备校准函数' 设备对象句柄

    '##################### AD的硬件参数操作函数 ###########################
    Declare Function PCI8510_SaveParaAD Lib "PCI8510_64" (ByVal hDevice As IntPtr, ByRef pADPara As PCI8510_PARA_AD) As Boolean  ' 将当前的AD采样参数保存至系统中
    Declare Function PCI8510_LoadParaAD Lib "PCI8510_64" (ByVal hDevice As IntPtr, ByRef pADPara As PCI8510_PARA_AD) As Boolean  ' 将AD采样参数从系统中读出
    Declare Function PCI8510_ResetParaAD Lib "PCI8510_64" (ByVal hDevice As IntPtr, ByRef pADPara As PCI8510_PARA_AD) As Boolean  ' 将AD采样参数恢复至出厂默认值
    ...
    Declare Function PCI8510_SetDeviceDO Lib "PCI8510_64" ( _
          ByVal hDevice As IntPtr, _
          ByVal bDOSts() As Byte) As Boolean    ' 输出开关量状态' 设备句柄	' 开关状态

    Declare Function PCI8510_GetDeviceDI Lib "PCI8510_64" ( _
             ByVal hDevice As IntPtr, _
             ByVal bDISts() As Byte) As Boolean    ' 取得开关量状态    ' 设备句柄		' 开关状态	

    Declare Function PCI8510_RetDeviceDO Lib "PCI8510_64" ( _
             ByVal hDevice As IntPtr, _
    ...
    Declare Function PCI8510_WriteRegisterByte Lib "PCI8510_64" ( _
         ByVal hDevice As IntPtr, _
    ...
    Declare Function PCI8510_WriteRegisterWord Lib "PCI8510_64" ( _
            ByVal hDevice As IntPtr, _
            ByVal LinearAddr As Int64, _
            ByVal OffsetBytes As UInt32, _
            ByVal Value As UInt16) As Boolean ' 写双字节数据（其余同上）

    Declare Function PCI8510_WriteRegisterULong Lib "PCI8510_64" ( _
            ByVal hDevice As IntPtr, _
            ByVal LinearAddr As Int64, _
            ByVal OffsetBytes As UInt32, _
            ByVal Value As UInt32) As Boolean ' 写四节字数据（其余同上）

    Declare Function PCI8510_ReadRegisterByte Lib "PCI8510_64" ( _
         ByVal hDevice As IntPtr, _
         ByVal LinearAddr As Int64, _
         ByVal OffsetBytes As UInt32) As Byte ' 读入单字节数据（其余同上）

    Declare Function PCI8510_ReadRegisterWord Lib "PCI8510_64" ( _
         ByVal hDevice As IntPtr, _
         ByVal LinearAddr As Int64, _
         ByVal OffsetBytes As UInt32) As UInt16 ' 读入双字节数据（其余同上）

    Declare Function PCI8510_ReadRegisterULong Lib "PCI8510_64" ( _
         ByVal hDevice As IntPtr, _
         ByVal LinearAddr As Int64, _
    ...
    Declare Function PCI8510_WritePortByte Lib "PCI8510_64" ( _
         ByVal hDevice As IntPtr, _
         ByVal nPort As Int64, _
         ByVal Value As Byte) As Boolean         ' 8位写端口' 设备句柄(由CreateDevice创建)	' 端口地址' 写出的8位整型数据
    Declare Function PCI8510_WritePortWord Lib "PCI8510_64" (ByVal hDevice As IntPtr, ByVal nPort As Int64, ByVal Value As UInt16) As Boolean
    Declare Function PCI8510_WritePortULong Lib "PCI8510_64" (ByVal hDevice As IntPtr, ByVal nPort As Int64, ByVal Value As UInt32) As Boolean

    Declare Function PCI8510_ReadPortByte Lib "PCI8510_64" (ByVal hDevice As IntPtr, ByVal nPort As Int64) As Byte
    Declare Function PCI8510_ReadPortWord Lib "PCI8510_64" (ByVal hDevice As IntPtr, ByVal nPort As Int64) As UInt16
    Declare Function PCI8510_ReadPortULong Lib "PCI8510_64" (ByVal hDevice As IntPtr, ByVal nPort As Int64) As UInt32
    ' 如果您要在用户模式下，直接访问硬件端口，请安装并使用ISA\CommUser下的驱动ReadPortByteEx等函数

  
    '########################### 线程操作函数 ######################################
    Declare Function PCI8510_CreateSystemEvent Lib "PCI8510_64" () As IntPtr  ' 创建内核事件对象，供InitDeviceDmaAD和VB子线程等函数使用
    Declare Function PCI8510_ReleaseSystemEvent Lib "PCI8510_64" (ByVal hEvent As IntPtr) As Boolean  ' 释放内核事件对象
