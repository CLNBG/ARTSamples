Module PCI9757
    ...
    '######################## 设备对象管理函数 ##############################
    Declare Function PCI9757_CreateDevice Lib "PCI9757_64" (Optional ByVal DeviceLgcID As Int32 = 0) As Int32     ' 用逻辑号创建设备对象
    Declare Function PCI9757_GetDeviceCount Lib "PCI9757_64" (ByVal hDevice As Int32) As Int32 ' 取得设备总台数
    Declare Function PCI9757_GetDeviceCurrentID Lib "PCI9757_64" (ByVal hDevice As Int32, ByRef DeviceLgcID As Int32, ByRef DevicePhysID As Int32) As Boolean
    Declare Function PCI9757_ListDeviceDlg Lib "PCI9757_64" (ByVal hDevice As Int32) As Boolean ' 以对话框窗体方式列表系统当中的所有的该PCI设备
    Declare Function PCI9757_ReleaseDevice Lib "PCI9757_64" (ByVal hDevice As Int32) As Boolean ' 仅释放设备对象
    ...
    ' AD通用函数
    Declare Function PCI9757_Calibration Lib "PCI9757_64" ( _
           ByVal hDevice As Int32) As Boolean   'AD校准

    ' AD程序查询方式函数
    Declare Function PCI9757_InitDeviceProAD Lib "PCI9757_64" ( _
            ByVal hDevice As Int32, _
            ByRef pADPara As PCI9757_PARA_AD) As Boolean ' 初始化设备，当返回TRUE后,设备即准备就绪.

    Declare Function PCI9757_StartDeviceProAD Lib "PCI9757_64" ( _
         ByVal hDevice As Int32) As Boolean       ' 在初始化之后，启动设备

    Declare Function PCI9757_ReadDeviceProAD_Npt Lib "PCI9757_64" ( _
            ByVal hDevice As Int32, _
    ...
    Declare Function PCI9757_GetDevStatusProAD Lib "PCI9757_64" ( _
            ByVal hDevice As Int32, _
            ByRef pADStatus As PCI9757_STATUS_AD) As Boolean ' 在AD采样过程中取得设备的各种状态,返回值表示函数是否成功

    Declare Function PCI9757_ReadDeviceProAD_Half Lib "PCI9757_64" ( _
            ByVal hDevice As Int32, _
            ByVal ADBuffer() As Int32, _
            ByVal nReadSizeWords As Int32, _
            ByRef nRetSizeWords As Int32) As Boolean ' 当AD标志有效时，用此函数读取设备上的AD数据(程序半满方式)

    Declare Function PCI9757_StopDeviceProAD Lib "PCI9757_64" ( _
         ByVal hDevice As Int32) As Boolean       ' 在启动设备之后，暂停设备

    Declare Function PCI9757_ReleaseDeviceProAD Lib "PCI9757_64" ( _
         ByVal hDevice As Int32) As Boolean       ' 关闭AD设备,禁止传输,且释放资源


    '##################### AD数据读取函数(DMA方式)(上层用户函数) ###########################
    ' AD直接内存(DMA)方式函数
    Declare Function PCI9757_InitDeviceDmaAD Lib "PCI9757_64" ( _
            ByVal hDevice As Int32, _
    ...
    Declare Function PCI9757_StartDeviceDmaAD Lib "PCI9757_64" ( _
         ByVal hDevice As Int32) As Boolean       ' 在初始化之后，启动设备

    Declare Function PCI9757_GetDevStatusDmaAD Lib "PCI9757_64" ( _
            ByVal hDevice As Int32, _
            ByRef pDMAStatus As PCI9757_STATUS_DMA) As Boolean   ' 在AD采样过程中取得DMA的有关状态,返回值表示函数是否成功

    Declare Function PCI9757_SetDevStatusDmaAD Lib "PCI9757_64" ( _
            ByVal hDevice As Int32, _
            ByVal iClrBufferID As Int32) As Boolean      ' 在AD采样过程中设置DMA的有关状态,返回值表示函数是否成功

    Declare Function PCI9757_StopDeviceDmaAD Lib "PCI9757_64" ( _
         ByVal hDevice As Int32) As Boolean       ' 在启动设备之后，暂停设备

    Declare Function PCI9757_ReleaseDeviceDmaAD Lib "PCI9757_64" ( _
         ByVal hDevice As Int32) As Boolean       ' 关闭AD设备,禁止传输,且释放资源

    '##################### AD的硬件参数操作函数 ###########################
    Declare Function PCI9757_SaveParaAD Lib "PCI9757_64" (ByVal hDevice As Int32, ByRef pADPara As PCI9757_PARA_AD) As Boolean ' 将当前的AD采样参数保存至系统中
    Declare Function PCI9757_LoadParaAD Lib "PCI9757_64" (ByVal hDevice As Int32, ByRef pADPara As PCI9757_PARA_AD) As Boolean ' 将AD采样参数从系统中读出
    Declare Function PCI9757_ResetParaAD Lib "PCI9757_64" (ByVal hDevice As Int32, ByRef pADPara As PCI9757_PARA_AD) As Boolean ' 将AD采样参数恢复至出厂默认值

    '################# 内存映射寄存器直接操作及读写函数 ########################
    ' 适用于用户对本设备更直接、更特殊、更低层、更复杂的控制。比如根据特殊的
    ' 控制对象需要特殊的控制流程和控制效率时，则用户可以使用这些接口予以实现。
    Declare Function PCI9757_GetDeviceBar Lib "PCI9757_64" ( _
             ByVal hDevice As Int32, _
             ByRef pulPCIBar As UInt32) As Boolean   ' 取得指定的指定设备寄存器组BAR地址

    Declare Function PCI9757_WriteRegisterByte Lib "PCI9757_64" ( _
         ByVal hDevice As Int32, _
         ByVal LinearAddr As UInt32, _
         ByVal OffsetBytes As UInt32, _
         ByVal Value As Byte) As Boolean          ' 往指定寄存器空间位置写入单节字数据

    Declare Function PCI9757_WriteRegisterWord Lib "PCI9757_64" ( _
            ByVal hDevice As Int32, _
            ByVal LinearAddr As UInt32, _
            ByVal OffsetBytes As UInt32, _
            ByVal Value As UInt16) As Boolean           ' 写双字节数据（其余同上）

    Declare Function PCI9757_WriteRegisterULong Lib "PCI9757_64" ( _
            ByVal hDevice As Int32, _
            ByVal LinearAddr As UInt32, _
            ByVal OffsetBytes As UInt32, _
            ByVal Value As UInt32) As Boolean          ' 写四节字数据（其余同上）

    Declare Function PCI9757_ReadRegisterByte Lib "PCI9757_64" ( _
            ByVal hDevice As Int32, _
            ByVal LinearAddr As UInt32, _
            ByVal OffsetBytes As UInt32) As Byte       ' 读入单字节数据（其余同上）

    Declare Function PCI9757_ReadRegisterWord Lib "PCI9757_64" ( _
            ByVal hDevice As Int32, _
            ByVal LinearAddr As UInt32, _
            ByVal OffsetBytes As UInt32) As UInt16     ' 读入双字节数据（其余同上）

    Declare Function PCI9757_ReadRegisterULong Lib "PCI9757_64" ( _
            ByVal hDevice As Int32, _
            ByVal LinearAddr As UInt32, _
    ...
    Declare Function PCI9757_WritePortByte Lib "PCI9757_64" ( _
         ByVal hDevice As Int32, _
         ByVal nPort As UInt32, _
         ByVal Value As Byte) As Boolean          ' 8位写端口

    Declare Function PCI9757_WritePortWord Lib "PCI9757_64" (ByVal hDevice As Int32, ByVal nPort As UInt32, ByVal Value As UInt16) As Boolean
    Declare Function PCI9757_WritePortULong Lib "PCI9757_64" (ByVal hDevice As Int32, ByVal nPort As UInt32, ByVal Value As UInt32) As Boolean

    Declare Function PCI9757_ReadPortByte Lib "PCI9757_64" (ByVal hDevice As Int32, ByVal nPort As UInt32) As Byte
    Declare Function PCI9757_ReadPortWord Lib "PCI9757_64" (ByVal hDevice As Int32, ByVal nPort As UInt32) As UInt16
    Declare Function PCI9757_ReadPortULong Lib "PCI9757_64" (ByVal hDevice As Int32, ByVal nPort As UInt32) As UInt32
    ' 如果您要在用户模式下，直接访问硬件端口，请安装并使用ISA\CommUser下的驱动ReadPortByteEx等函数


    '########################### 线程操作函数 ######################################
    Declare Function PCI9757_CreateSystemEvent Lib "PCI9757_64" () As Int32 ' 创建内核事件对象，供InitDeviceDmaAD和VB子线程等函数使用
    Declare Function PCI9757_ReleaseSystemEvent Lib "PCI9757_64" (ByVal hEvent As Int32) As Boolean ' 释放内核事件对象
