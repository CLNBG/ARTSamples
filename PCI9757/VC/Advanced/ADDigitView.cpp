...
{	
	...
	case SHOW_MODE_DEC:	// 十进制显示
		_stprintf_s(str, _T("%s"), _T("  十进制显示数据(去掉无效位D14、D15)"));
		...
		{			
			...
			{				
				_stprintf_s(strIndex, _T("CH%d           ") , Channel);
	...
	case SHOW_MODE_HEX:	// 十六进制显示
		_stprintf_s(str, _T("%s") , _T("  十六进制原码显示数据"));
	...
	case SHOW_MODE_VOLT:	// 电压值显示
		_stprintf_s(str, _T("%s") , _T("  电压值显示数据"));
	...
	for (int RowY=rect.top+16; RowY<rect.bottom; RowY++)
	{
		...
		if (RowY%16 == 0)
		{
			Index = gl_Offset + RowY/16 - 1; // 数据点索引值
			if (Index>(MAX_SEGMENT_SIZE/2/gl_nChannelCount-1))
			{
				break;
			}
		 	DataIndex = Index * gl_nChannelCount;
 			 
			if (Index > gl_Offset + HALF_SIZE_WORDS/gl_nChannelCount-1) break;
			_stprintf_s(strIndex, _T("%4d") , Index); 
			...
			case SHOW_MODE_DEC: // 十进制偏移码				
				for (Channel=0; Channel<MAX_CHANNEL_COUNT; Channel++)
				{
					if (ADPara.bChannelArray[Channel] == TRUE)
					{
						ADLsb = (ADBuffer[gl_nDrawIndex][DataIndex])&MASK_MSB + ADCHZeroLsb[Channel]; // 从缓冲区中获得最终的原始码数据LSB
						ADData = ADLsb - gl_MiddleLsb[Channel]; // 将原始码数据减去平移电压码值
						_stprintf_s(str, _T(" %8d") , ADData); // 将AD数值转换成字符串以便显示
						pDC->TextOut(-rect.left-30+(TChannel+1)*70, rect.top+nTextHeight+36, str); // text out decimal value every channel
						TChannel++;
			...
			case SHOW_MODE_HEX: // 十六进制原码
				for (Channel=0; Channel<MAX_CHANNEL_COUNT; Channel++)
				{
					if (ADPara.bChannelArray[Channel] == TRUE)
					{
						...
						_stprintf_s(str, _T("%4x") , ADLsb&MASK_MSB);
						//_stprintf_s(str, _T("%d:%d:%4d", (ADLsb>>15)&0x0001, (ADLsb>>14)&0x0001, ADLsb);
						pDC->TextOut(-rect.left-30+(TChannel+1)*80, rect.top+nTextHeight+36, str); // text out hex value every channel
						TChannel++;
						if (TChannel>=gl_nChannelCount) TChannel = 0;
						DataIndex++;
					}					
			...
			case SHOW_MODE_VOLT: // 电压值
				for (Channel=0; Channel<MAX_CHANNEL_COUNT; Channel++)
				{
					if (ADPara.bChannelArray[Channel] == TRUE)
					{
						...
						{
							ADData = ADLsb - gl_MiddleLsb[Channel];
						}
						else
						{
							...
							case PCI9757_INPUT_N2500_P2500mV:
								ADData = ADLsb - AD_LSB_HALF /*+ ADCHZeroLsb[Channel]*/; // 将原始码数据减去平移电压码值和半幅偏移码值
								break;
							case PCI9757_INPUT_0_P10000mV:
							case PCI9757_INPUT_0_P5000mV:
								ADData = ADLsb; // 将原始码数据减去平移电压码值和半幅偏移码值
							    break;
							default:
							    break;
							}							
						...
						_stprintf_s(str, _T("  %8.2f") , fADVolt);
						pDC->TextOut(-rect.left-30+(TChannel+1)*70, rect.top+nTextHeight+36, str); // text out volt value
						TChannel++;
						if (TChannel>=gl_nChannelCount) TChannel = 0;
						DataIndex++;
					}
