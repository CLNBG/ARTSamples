...
#include "PCI9757/PCI9757.h"  // 包含驱动程序接口
//#############################################################
// 定义用于控制数据位数及量程的常数
#define AD_LSB_COUNT  65536.0       // LSB码数量
#define AD_LSB_HALF   32768          // 当取偏移码时，其原点位置
#define AD_VOLT_RANGE 20000.0		// 量程（mV）(注意必须带小数)
#define MASK_MSB      0xFFFF			// ^(0x02000)

#define DA_VOLT_RANGE 20000.0          // DA输出的量程(mV)
// 定义其他常量
#define DEVICE_NAME        "PCI9757"   // 定义设备名
#define MAX_FREQUENCY     1000000       // 最高频率Hz
#define MAX_AD_CHANNELS   4           // AD通道数
...
#define DEFAULT_DEVICE_NUM 0x9757   // 默认的设备编号
...
typedef struct _FILE_HEADER
{
	...
	LONG StaticOverFlow;					// 静态溢出标志
	LONG DynamicOverflow;					// 动态溢出标志
