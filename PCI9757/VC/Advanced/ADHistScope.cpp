...
void CADHistScope::InvalidateCtrl()
{
	...
	for (Channel=0; Channel<MAX_CHANNEL_COUNT; Channel++)
	{
		...
		case PCI9757_INPUT_N5000_P5000mV:  // ±5000mV
			m_dLowerLimit[Channel] = -5000.0;
			m_dUpperLimit[Channel] = 5000.0;
			break;
			
		case PCI9757_INPUT_N2500_P2500mV:  // ±5000mV
			m_dLowerLimit[Channel] = -2500.0;
			m_dUpperLimit[Channel] = 2500.0;
			break;
			
		case PCI9757_INPUT_0_P10000mV:    // 10000mV
			m_dLowerLimit[Channel] = 0;
			m_dUpperLimit[Channel] = 10000.0;
			break;					
...
void CADHistScope::OnPaint() 
{
	...
	TCHAR str[100];
	_stprintf_s(str, _T("%s") , _T("A线"));
	dc.SetTextColor(RGB(255, 0, 0));
	dc.TextOut(m_OldMoveX1, m_rectPlot.top+10, str);
	
	dc.SelectObject(&m_PenLineB);				    // 选区入线B的画笔
	dc.MoveTo(m_OldMoveX2, m_rectPlot.top);
	dc.LineTo(m_OldMoveX2, m_rectPlot.Height()+10); // 画线B
	_stprintf_s(str, _T("%s") , _T("B线"));
	dc.SetTextColor(RGB(255, 0, 0));
	dc.TextOut(m_OldMoveX2, m_rectPlot.top+10, str);
	if (!m_bAllChannel)
	{
...
void CADHistScope::DrawPoint()
{
	...
	if (m_dcPlot.GetSafeHdc() != NULL)
	{
		m_dcPlot.BitBlt(m_rectPlot.left, m_rectPlot.top+1, 
			...
			m_rectPlot.left+1, m_rectPlot.top+1, 
			SRCCOPY);		
...
void CADHistScope::OnSize(UINT nType, int cx, int cy) 
{
	...
	{ 
		...
		m_OldVoltageY = m_rectPlot.top + 20;
...
void CADHistScope::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	str.Format(_T("%d") , m_MouseOffset + pDoc->m_Offset/m_nChannelCount);
...
void CADHistScope::AppendPoly(int  Offset)
{
...
{	
	...
	m_Center = (int)(PerY/2.0)+m_rectPlot.top;
	int Channel, Index, StartX;
	PLONG  ptOffset; // 缓存指针
	int Offset = 0, DataY = 0;

	CADHistWaveView* pView = (CADHistWaveView*)GetParent();
	CADHistDoc* pHistDoc = (CADHistDoc*)pView->GetDocument();
	...
	if (gl_bTileWave) // 多通道平铺显示
	{
		for (Channel=0; Channel<m_nChannelCount; Channel++)
		{
			...
			DataY = (int)(((ptOffset[Offset])&MASK_MSB)/LsbOfPixel);
			pointxy[Channel][0].y = (int)(m_Center) - DataY;
			
			for (Index=0; Index<=(HALF_SIZE_WORDS/m_nChannelCount); Index++) // 初始化1024个点(创建时，位图的大小) 
			{
				pointxy[Channel][Index].x = StartX + Index;
				DataY = (int)((((((ptOffset[(Offset + Index) * m_nChannelCount + Channel]))&MASK_MSB) - AD_LSB_HALF) / LsbOfPixel));
				pointxy[Channel][Index].y = (int)(m_Center) - DataY;
			}
			m_Center += PerY;
		} 
	}
	else // 多通道叠加显示
	{
		for (Channel=0; Channel<m_nChannelCount; Channel++)
		{
			...
			m_Center = (int)(m_rectPlot.Height() / 2) + m_rectPlot.top;
...
{	
	...
	CADHistWaveView* pView = (CADHistWaveView*)GetParent();
	CADHistDoc* pDoc = (CADHistDoc*)pView->GetDocument();
	int nDrawCount = pDoc->m_ReadDataSize/m_nChannelCount; 

	if (nDrawCount > m_rectClient.Width())
	{
		nDrawCount = m_rectClient.Width();
	}
	...
	{	
		...
		int StartX = m_rectPlot.left;
		m_Center = (int)(m_nPlotHeight / 2) + m_rectPlot.top;
		ptOffset = &ADBuffer[gl_nDrawIndex][m_Offset]; // 指针的偏移量
		for (int Index=0; Index<=m_nPlotWidth; Index++)	
		{
			pointTemp[Index].x = StartX + Index;
			DataY = (int)(((((ptOffset[Index])&MASK_MSB) - AD_LSB_HALF - gl_MiddleLsb[m_nChannelNum]) / LsbOfPixel));
			pointTemp[Index].y = (int)(m_Center) - DataY;
		}
...
//	pEditTime->SetWindowText(strCountTimer);
	...
	int m_Center = m_rectPlot.Height()/2 + m_rectPlot.top;
	int nOffset = m_Center - m_OldVoltageY;
...
void CADHistScope::DrawMoveLine(CDC* pDC, CPoint point)
{
	...
	case 0: // 同时选择线1线2
		...
		pDC->TextOut(m_OldMoveX1, m_rectPlot.top+10, str);
	...
	case 1: // 选择的线条1
		...
		_stprintf_s(str, _T("%s") , _T("A线"));
		pDC->SetTextColor(RGB(255, 0, 0));
		pDC->TextOut(point.x, m_rectPlot.top+10, str);
		pDC->SelectObject(pOldPen); // 恢复旧的画笔
		break;
	...
	case 2: // 选择的线条2
		...
		_stprintf_s(str, _T("%s") , _T("B线"));
		pDC->SetTextColor(RGB(0, 0, 0));
		pDC->TextOut(m_OldMoveX2, m_rectPlot.top+10, str);
...
void CADHistScope::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
	InvalidateCtrl();
