...
BOOL CSysApp::InitInstance()
{
	...
	int DeviceCount;
	hDevice = PCI9757_CreateDevice(0); // 创建设备对象，保存在App中，可供其他任何子窗体或子功能使用
...
{	
	...
	if (File.Open(lpszFileName, CFile::modeRead)) // 如果文件打开成功
	{
		...
		if ((FileHeader.DeviceNum) != DEFAULT_DEVICE_NUM)
		{
			...
			if (AfxMessageBox(_T("此文件并非PCI9757板卡配套文件, 是否继续?") , MB_OKCANCEL) == IDCANCEL)
				return NULL;
...
int CSysApp::ExitInstance() 
{
	...
	{
		PCI9757_ReleaseDevice(m_hDevice ); // 释放设备对象
		m_hDevice = INVALID_HANDLE_VALUE;
	}
...
{
	// TODO: Add your command handler code here
	PCI9757_ListDeviceDlg(m_hDevice); // 列表该类设备
}
