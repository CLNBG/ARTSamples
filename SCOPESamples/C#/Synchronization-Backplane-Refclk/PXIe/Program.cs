///////////////////////////////////////////////////////////////////
// This example program demonstrates multi cards synchronization in PXIe bus using backplane reference clock.
...
namespace Sys
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            triggerSynPulseSource = ART_SCOPE.ARTSCOPE_VAL_TRIG_SYNC_0;
            ...
            if (ART_SCOPE.ArtScope_ConfigureTriggerSyncSrc(taskHandle_master, triggerSynPulseSource) < 0)
            {
                bError = true;
                goto Error_close;
            }
            ...
            if (ART_SCOPE.ArtScope_ConfigureClock(taskHandle_master, ART_SCOPE.ARTSCOPE_VAL_CLK10M_PXI, 1, 1) < 0)
            {
                bError = true;
                goto Error_close;
            }
            ...
            UInt16[] waveformPtr_master = new UInt16[actualRecordLength_master * numWfms_master];
            ...
            if (ART_SCOPE.ArtScope_ConfigureTriggerSyncSrc(taskHandle_slave, triggerSynPulseSource) < 0)
            {
                bError = true;
                goto Error_close;
            }

            if (ART_SCOPE.ArtScope_ConfigureTriggerEdge(taskHandle_slave, ART_SCOPE.ARTSCOPE_VAL_TRIGSRC_SYNC, triggerLevel, triggerSlope, triggerCoupling, triggerCount, triggerSensitivity) < 0)
            {
                bError = true;
                goto Error_close;
            }

            // also can set ARTSCOPE_VAL_CLK100M_PXIE
            if (ART_SCOPE.ArtScope_ConfigureClock(taskHandle_master, ART_SCOPE.ARTSCOPE_VAL_CLK10M_PXI, 1, 0) < 0)
            {
                bError = true;
                goto Error_close;
            }
            ...
            UInt16[] waveformPtr_slave = new UInt16[actualRecordLength_slave * numWfms_slave];
            ...
            // Start thread to read data of the slave card
            ManualResetEvent handle_slave = new ManualResetEvent(false);
            manualEvents.Add(handle_slave);
            ReadThread_Slave slavereadclass = new ReadThread_Slave(taskHandle_slave, timeout, actualRecordLength_slave, numWfms_slave, waveformPtr_slave, handle_slave);
            ...
            // Start thread to read data of the master card
            ManualResetEvent handle_master = new ManualResetEvent(false);
            manualEvents.Add(handle_master);
            ReadThread_Master masterreadclass = new ReadThread_Master(taskHandle_master, timeout, actualRecordLength_master, numWfms_master, waveformPtr_master, handle_master);
