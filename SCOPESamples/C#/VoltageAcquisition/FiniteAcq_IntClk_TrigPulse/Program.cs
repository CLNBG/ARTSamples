...
namespace Sys
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Int32 verticalRange = ART_SCOPE.ARTSCOPE_RANGE_40Vpp;
            ...
            Int32 triggerSlope = ART_SCOPE.ARTSCOPE_VAL_TRIGDIR_PULSE_GREATER;
            Int32 triggerCoupling = ART_SCOPE.ARTSCOPE_VAL_TRIGCOUP_DC;
            Int32 triggerPolarity = ART_SCOPE.ARTSCOPE_VAL_TRIGPOLAR_NEGATIVE;
            ...
            if (ART_SCOPE.ArtScope_ConfigureTriggerPulse(taskHandle, triggerSource, triggerLevel, triggerPolarity, triggerSlope, pulseWidth, triggerCoupling, triggerCount, triggerSensitivity) < 0)
            {
                bError = true;
                goto Error_close;
            }
            ...
            double[] waveformPtr = new double[actualRecordLength * numWfms];
            ...
            if (ART_SCOPE.ArtScope_FetchVoltage(taskHandle, timeout, actualRecordLength, waveformPtr, ref wfmInfo) < 0)
            {
                Console.WriteLine("Fetch voltage error!");
                goto Error_label;
            }
            for (k = 0; k < numWfms; k++)
            {
                Console.Write("{0,8:f2}(mV)\t", waveformPtr[k]);
            }
            Console.WriteLine("");
