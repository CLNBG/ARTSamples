...
#配置边沿触发
triggerSource = TriggerSource.TRIGSRC_CH0               # trigger source
triggerLevel = 0.0                                      # trigger level
triggerSlope = TriggerSlope.TRIGDIR_NEGATIVE            # trigger slope
triggerCoupling = TriggerCoupling.TRIGCOUP_DC           # trigger coupling
triggerCount = 1                                        # trigger count
triggerSensitivity = 50                                 # trigger sensitivity
error_code = Functions.ArtScope_ConfigureTriggerEdge(taskHandle, triggerSource, triggerLevel, triggerSlope, triggerCoupling, triggerCount, triggerSensitivity)
if error_code < 0:
    Functions.ArtScope_Close(taskHandle)
    check_for_error(error_code)
...
waveformPtr = numpy.zeros(sumLength, dtype=numpy.uint16)
...
while True:
    if msvcrt.kbhit():
        if ord(msvcrt.getch()) != None:
            break
    error_code = Functions.ArtScope_FetchBinary16(taskHandle, timeout, readLength, waveformPtr, wfmInfo)
    if error_code < 0:
    ...
    #print one data for one channel
    for index in range(numWfms.value):
        fVolt = wfmInfo.fLsbs[index%numWfms.value]*(waveformPtr[index]&wfmInfo.wMaxLSB) - wfmInfo.rangevalue[index%numWfms.value]
        print("%8.2f (mV)\t" %(fVolt))
    print("\n")
