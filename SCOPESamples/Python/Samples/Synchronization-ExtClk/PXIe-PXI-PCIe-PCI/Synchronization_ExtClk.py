...
def ReadThread_Master(taskhandle, timeout, samplelens, databuffer, wfmInfo):
    while True:
        if msvcrt.kbhit():
            if ord(msvcrt.getch()) != None:
                break
        error_code = Functions.ArtScope_FetchBinary16(taskhandle, timeout, samplelens, databuffer, wfmInfo)
        if error_code < 0:
            Functions.ArtScope_StopAcquisition(taskhandle)
            Functions.ArtScope_Close(taskhandle)
            check_for_error(taskhandle)
        # 每个通道打印一个点
...
def ReadThread_Slave(taskhandle, timeout, samplelens, databuffer, wfmInfo):
    while True:
        if msvcrt.kbhit():
            if ord(msvcrt.getch()) != None:
                break
...
error_code = Functions.ArtScope_ConfigureTriggerSyncSrc(taskHandle_master)
if error_code < 0:
...
#set external clock
inputClockSource = InputClockSource.CLK_IN
...
#set sychronous trigger source
error_code = Functions.ArtScope_ConfigureTriggerSyncSrc(taskHandle_slave)
if error_code < 0:
...
error_code = Functions.ArtScope_ConfigureClock(taskHandle_slave, inputClockSource, 1, 1)
if error_code < 0:
    Functions.ArtScope_Close(taskHandle_slave)
    check_for_error(error_code)
