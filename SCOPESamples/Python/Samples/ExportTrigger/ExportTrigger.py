...
#set trigger out
triggerOutWidth = 100
triggerOutPolarity = TriggerOutPolarity.TRIGOUT_POLAR_NEGATIVE
error_code = Functions.ArtScope_ExportTrigger(taskHandle, triggerOutWidth, triggerOutPolarity)
if error_code < 0:
    Functions.ArtScope_Close(taskHandle)
    check_for_error(error_code)
...
while True:
    ...
    for index in range(numWfms.value):
        print("%d(Binary)\t" %(waveformPtr[index]))
    print("\n")
