...
def ReadThread_Master(taskhandle, readtimeout, samplelens, databuffer, wfminfo):
    while True:
        if msvcrt.kbhit():
            if ord(msvcrt.getch()) != None:
                break
        error_codet = Functions.ArtScope_FetchBinary16(taskhandle, readtimeout, samplelens, databuffer, wfminfo)
        if error_code < 0:
    ...
    # stop
    error_codet = Functions.ArtScope_StopAcquisition(taskhandle)
    # release
    error_codet = Functions.ArtScope_ReleaseAcquisition(taskhandle)
    # close
    error_codet = Functions.ArtScope_Close(taskhandle)
...
inputClockSource = InputClockSource.CLK10M_PXI
...
#set clock parameters
inputClockSource = InputClockSource.ARTSCOPE_VAL_CLK10M_PXI
