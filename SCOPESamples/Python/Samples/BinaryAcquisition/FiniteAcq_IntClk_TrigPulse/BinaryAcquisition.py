...
verticalRange = InputRange.RANGE_16VPP                  # range
...
triggerLevel = 0.0
triggerPolarity = TriggerPolarity.TRIGPOLAR_NEGATIVE
triggerSlope = TriggerSlope.TRIGDIR_NEGATIVE
pulseWidth = 50
triggerCoupling = CouplingType.DC
triggerCount = 1
triggerSensitivity = 50
error_code = Functions.ArtScope_ConfigureTriggerPulse(taskHandle, triggerSource, triggerLevel, triggerPolarity, triggerSlope, pulseWidth, triggerCoupling, triggerCount, triggerSensitivity)
if error_code < 0:
    Functions.ArtScope_Close(taskHandle)
    check_for_error(error_code)
...
waveformPtr = numpy.zeros(sumLength, dtype=numpy.uint8)
...
error_code = Functions.ArtScope_FetchBinary8(taskHandle, timeout, readLength, waveformPtr, wfmInfo)
if error_code < 0:
