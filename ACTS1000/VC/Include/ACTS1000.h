...
// PXIE/PCIE 8506 4CH 16Bit 40Msps
// PXIE/PCIE 8512 4CH 12Bit 80Msps
// PXIE/PCIE 8514 4CH 14Bit 80Msps
// PXIE/PCIE 8516 4CH 14Bit 80Msps
// PXIE/PCIE 8526 4CH 14Bit 125Msps
// PXIE/PCIE 8912 2CH 12Bit 250Msps,支持阻抗选择
// PXIE/PCIE 8914 2CH 14Bit 250Msps,支持阻抗选择
// PXIE/PCIE 8916 2CH 16Bit 250Msps,支持阻抗选择
// PXIE/PCIE 8562 4CH 12Bit 250Msps,支持阻抗选择
// PXIE/PCIE 8564 4CH 14Bit 250Msps,支持阻抗选择
// PXIE/PCIE 8566 4CH 16Bit 250Msps,支持阻抗选择
// PXIE/PCIE 8922 2CH 12Bit 500Msps,支持阻抗选择
...
// PXIE/PCIE 8582A 8CH 12Bit 100Msps,PFI选择(触发输出/触发输入)
// PXIE/PCIE 8584A 8CH 14Bit 100Msps,PFI选择(触发输出/触发输入)
// PXIE/PCIE 8586A 8CH 16Bit 100Msps,PFI选择(触发输出/触发输入)
// PXIE/PCIE 858C 8CH 16Bit 100Msps,PFI选择(触发输出/触发输入),不支持外时钟,不支持时钟输出
...
typedef struct _ACTS1000_PARA_AD     
{
	...
	LONG PFISel;			// PFI功能选择, 详见下面常量定义(仅板卡8582 8584 8586 8582A 8584A 8586A支持)
	...
	LONG bClkOutEn;			// 时钟是否输出,TRUE输出,(板卡8582 8584 8586不支持)
	LONG ClkOutSel;			// 时钟输出选择,(板卡8582 8584 8586不支持)
...
const long ACTS1000_INPUT_N5000_P5000mV		= 0x00; // ±5000mV,PCIE8502D为±10V
const long ACTS1000_INPUT_N1000_P1000mV		= 0x01; // ±1000mV
const long ACTS1000_INPUT_N10000_P10000mV	= 0x02; // ±10000mV
...
// AD参数ACTS1000_PARA_AD中的PFISel PFI功能选择所使用的选项(仅8582 8584 8586 8582A 8584A 8586A板卡支持)
...
const long ACTS1000_RECLK_EXT_10M			= 0x01;	// 外部10M（CLK_IN）(板卡8582 8584 8586不支持)
...
const long ACTS1000_RECLK_EXT				= 0x01;	// 外部时钟（CLK_IN）(板卡8582 8584 8586不支持)
...
extern "C" {
	...
	//######################## 常规通用函数 #################################
	// 适用于本设备的最基本操作
	HANDLE DEVAPI FAR PASCAL ACTS1000_CreateDevice(int DeviceID = 0);		// 创建设备对象
	...
	BOOL DEVAPI FAR PASCAL ACTS1000_GetSupportRangeInfo(						// 获取支持的量程
												HANDLE hDevice,					// 设备对象
												PULONG pSampRangeCount,			// 返回支持的量程总数
												ULONG pSampRangeVal[8]);		// 返回对应量程值
																				// 例:858C,pSampRangeCount返回2表示支持两个量程，pSampRangeVal[0]=1表示支持正负1V,pSampRangeVal[1]=2表示支持正负10V
	...
	BOOL DEVAPI FAR PASCAL ACTS1000_ReadDeviceAD(								// DMA方式读AD数据
												HANDLE hDevice,					// 设备句柄
												PWORD pADBuffer,				// 将用于接受数据的用户缓冲区(该区必须开辟大于nReadSizeWords字的空间)
												ULONG nReadSizeWords,			// 读取长度
												PULONG nRetSizeWords,			// 返回实际读取的数据长度
												PULONG pAvailSampsPoints,		// 剩余点数
												double fTimeout);				// 超时时间

	BOOL DEVAPI FAR PASCAL ACTS1000_ReadAnalog(									// 读取采样数据(电压数据序列)
												HANDLE hDevice,					// 设备句柄
												float fAnlgArray[],				// 将用于接受数据的用户缓冲区(该区必须开辟大于nReadSizeWords字的空间)
												ULONG nReadSizeWords,			// 读取长度
												PULONG nRetSizeWords,			// 返回实际读取的数据长度
												PULONG pAvailSampsPoints,		// 剩余点数
												double fTimeout);				// 超时时间

	BOOL DEVAPI FAR PASCAL ACTS1000_ReadAnalogF64(								// 读取采样数据(电压数据序列)
												HANDLE hDevice,					// 设备句柄
												double dAnlgArray[],			// 将用于接受数据的用户缓冲区(该区必须开辟大nReadSizeWords字的空间)
												ULONG nReadSizeWords,			// 读取长度
												PULONG nRetSizeWords,			// 返回实际读取的数据长度
												PULONG pAvailSampsPoints,		// 剩余点数
												double fTimeout);				// 超时时间
	...
	BOOL DEVAPI FAR PASCAL ACTS1000_GetDMAMemorySize(							// 初始后获取实际内存大小,单位KB
												HANDLE hDevice,					// 设备句柄
												ULONG* pDMAMemSize);			// DMA内存大小,单位KB

	// 定义回调函数
	// hDevice-设备句柄
	// nSamplesPerChan-每通道的采样点数 此参数与ACTS1000_RegisterEveryNSamplesEvent接口中nSamplesPerChan的值一致
	// nChannelCount-参与采集的通道个数
	// callbackData-暂未使用 暂且保留
	// 函数返回值为0表示正常 其余值表示有错误存在
	typedef LONG (*ACTS1000_EveryNSamplesEventCallbackPtr)(HANDLE hDevice, ULONG nSamplesPerChan, ULONG nChannelCount, void *callbackData);

	BOOL DEVAPI FAR PASCAL ACTS1000_RegisterEveryNSamplesEvent(					// 注册回调函数接口
												HANDLE hDevice,					// 设备对象句柄,它由DEV_Create()函数创建
												ULONG nSamplesPerChan,			// 每通道采集点数 此参数决定调用读取接口时 读取的数据点数
												ULONG options,					// 保留
												ACTS1000_EveryNSamplesEventCallbackPtr callbackFunction,// 回调函数指针
												void *callbackData);			// 保留
	...
	BOOL DEVAPI FAR PASCAL ACTS1000_GetBusInfo(							// 获得设备总线信息
												...
												ULONG* pRevisionID);	// 返回RevisionID,0xD0:如PCIE8502D,0x01:如PCIE8582A 
...
#ifndef _ACTS1000_DRIVER_
	#ifndef _WIN64
		#pragma comment(lib, "ACTS1000_32.lib")
		#pragma message("======== Welcome to use our art company's products!")
		#pragma message("======== Automatically linking with ACTS1000_32.dll...")
		#pragma message("======== Successfully linked with ACTS1000_32.dll")
	#else
		#pragma comment(lib, "ACTS1000_64.lib")
		#pragma message("======== Welcome to use our art company's products!")
		#pragma message("======== Automatically linking with ACTS1000_64.dll...")
		#pragma message("======== Successfully linked with ACTS1000_64.dll")
	#endif
