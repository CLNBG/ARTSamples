...
int CCfgTabCtrlBar::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_StartDeviceButton.Create(_T("开始数据采集"), WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON | BS_DEFPUSHBUTTON , StartButtonRect, this, IDM_StartDeviceAD);
	//m_StopDeviceButton = new CButton();
	m_StopDeviceButton.Create(_T("停止数据采集"), WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON  , StopButtonRect, this, IDM_StopDeviceAD);
...
void CCfgTabCtrlBar::OnSize(UINT nType, int cx, int cy) 
{
	...
	for (POSITION pos=m_views.GetHeadPosition(); pos; m_views.GetNext(pos))
	{
		...
		pWnd->MoveWindow(9, 99, cx-18, bottom-25); // 在CControlBar在移动视图类的控件位置
	}
}
...
{	
	...
	int nViews = (int)m_views.GetCount();
	if (nViews!=1)
	{
