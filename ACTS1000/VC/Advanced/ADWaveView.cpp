...
void CADWaveView::OnInitialUpdate()
{
	...
	m_ADScopeCtrl.SetXUnits(_T("X"));
	m_ADScopeCtrl.SetYUnits(_T("Y"));
...
void CADWaveView::AnalyzeAccurate()//精度分析
{
	...
	int m_TriggerLsb;
	m_TriggerLsb=(int)(m_TriggerVolt*(gl_fLsbCount/VOLT_RANGE));
	for(ULONG i=0;i<gl_ReadSizeWords;i++)
	{
		m_TemporyVolt=(int)((ADBuffer[CurrentIndex][i])-gl_MiddleLsb[i%MAX_CHANNEL_COUNT]);
	...
	StrAllCount.Format(_T("%3.2e"),m_AnalyzeAllCount);
	StrOverCount.Format(_T("%d"),m_OverLimitCount);
	StrOverRatio.Format(_T("%4.4f"),m_OverLimitCount*100/m_AnalyzeAllCount);
...
void CADWaveView::OnSize(UINT nType, int cx, int cy) 
{
	...
	middle1=(float)((((gl_ScreenVolume)/VOLT_RANGE)*gl_fLsbCount)/(PerY-5));//每像素对应的码值    
	CRect rect;
	GetClientRect(rect);
