...
{ // 这里的m_nClientWidth, m_ClientHeight也是取决于m_rectClient的大小
	...
	dc.TextOut(m_OldMoveX1, m_rectPlot.Height()+13, _T("触发点"));
	...
	if(m_rectPlot.PtInRect(point))
	{
		...
		if(gl_LBtnDown) 
		{
			...
			case 1: // 选择的线条1
				...
				dc.SetTextColor(RGB(255, 255, 0));
				dc.TextOut(point.x, m_rectPlot.Height()+13, _T("触发点"));
...
void CADScopeCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	...
	if(m_bDrawPoly == TRUE && point.x > m_rectPlot.left && gl_bTileWave) // 当鼠标点在边框内时
	{
		...
		{	
			...
			tempCount = m_nChannelCount;				
			CString str;
			str.Format(_T("总共-%d-通道 "), m_nChannelCount); 
			this->SetYUnits(str);
...
{	
	...
	LsbOfPixel=(float)(((m_ScreenVolume/VOLT_RANGE)*gl_fLsbCount)/(PerY));//每像素对应的码值    
		...
		if (!m_bDrawComp)
		{
			...
			{								
				if (ADPara.bChannelArray[Channel] == TRUE)
				{
					...
					{
						LsbOfPixel = (float)(((gl_ScreenVolume*1.0/gl_InputRange[Channel])*gl_fLsbCount)/(m_rectPlot.Height()));//每像素对应的码值    
						//LsbOfPixel = (float)(((m_ScreenVolume/VOLT_RANGE)*gl_fLsbCount)/(m_rectPlot.Height()));//每像素对应的码值    
...
{	
	...
	{	
		...
		if (!m_bDrawComp)
		{
			...
			LsbOfPixel = (float)(((gl_ScreenVolume*gl_fLsbCount/gl_InputRange[m_nChannelNum]))/(m_rectPlot.Height()));//每像素对应的码值  
...
void CADScopeCtrl::DrawAllChannelText(CDC *pDC)
{
	...
	for(int i=0; i<MAX_CHANNEL_COUNT; i++)
	{
		...
		str.Format (_T("%.*lf V"), m_nYDecimals, m_dUpperLimit[i]/1000.0); // 正电压值
		pDC->TextOut (m_rectPlot.left-4, (int)(m_rectPlot.top+hight*nCount+8), str); 
		
		pDC->SetTextAlign (TA_RIGHT|TA_BASELINE);
		str.Format (_T("%.*lf V"), m_nYDecimals, m_dLowerLimit[i]/1000.0); // 负电压值
...
void CADScopeCtrl::DrawSingleCHText(CDC *pDC, int nChannelNum)
{
	...
	if(nChannelNum != 0xFFFF)
	{
		...
		str.Format (_T("%.*lf mV"), 0, m_dUpperLimit[nChannelNum]); // 正电压值
		pDC->TextOut (m_rectPlot.left-4, m_rectPlot.top, str); 
		pDC->SetTextAlign (TA_RIGHT|TA_BASELINE) ;
		str.Format (_T("%.*lf mV"), 0, m_dLowerLimit[nChannelNum]); // 负电压值
	    pDC->TextOut (m_rectPlot.left-4, m_rectPlot.top+m_nPlotHeight, str);
	}
	else
	{
		pDC->SetTextColor(m_crGridColor);
	    str.Format (_T("%.*lf mV"), 0, m_dUpperLimit[0]); // 正电压值
	    //pDC->TextOut (m_rectPlot.left-4, m_rectPlot.top, str); 
	    pDC->SetTextAlign (TA_RIGHT|TA_BASELINE) ;
	    str.Format (_T("%.*lf mV"), 0, m_dLowerLimit[0]); // 负电压值
		//pDC->TextOut (m_rectPlot.left-4, m_rectPlot.top+m_nPlotHeight, str);
	}
