...
CSysApp theApp;
ULONGLONG gl_ReadSizeWords;	// 读入的数据长度
...
BOOL CSysApp::InitInstance()
{
	...
	while (TRUE)
	{
		...
		{ 	
			...
			{			
				AfxMessageBox(L"对不起，您的所有设备已被相应程序管理，您不能再创建新实例,或设备不存在", MB_ICONWARNING, 0);
			}
			break;
		}
	}
	...
	/*int DeviceCount;
	hDevice = ACTS1000_CreateDevice(0); // 创建设备对象，保存在App中，可供其他任何子窗体或子功能使用
	...
	for(int i=0; i<DeviceCount; i++)
	{
		...
		// 创建互斥对象
		m_hMutex=::CreateMutex(NULL, NULL, strMutex);  // m_pszExeName为本程序的执行名
		if(GetLastError()==ERROR_ALREADY_EXISTS)  // 第二次创建应用程序
		{
			bCreate=FALSE;
			continue;  // 如果已经创建，则继续下一个设备的应用程序创建
	...
	if(DeviceCount!=0)
	{
		...
		{
			AfxMessageBox(_T("对不起，您的所有设备已被相应程序管理，您不能再创建新实例..."),MB_ICONWARNING,0);
			return FALSE; 
		}
		m_hDevice = ACTS1000_CreateDevice(m_CurrentDeviceID);
	}
	*/
	ACTS1000_GetMainInfo(m_hDevice,&gl_ADMainInfo);			// DDR2的长度(单位：MB)
...
// 	}
	...
	int a =File.Read((WORD*)&m_Header,sizeof(m_Header)); // 文件信息由HistDigitView类将其置在HistDataFrm的对话框条上OnInitialUpdate
	File.Close();
	if (a == 0)
	{
		AfxMessageBox(L"文件大小为0");
		return;
	}
...
// 	
	...
	{
		if(AfxMessageBox(_T("对不起，该文件不能解析, 是否继续"),MB_YESNO, 0)==IDNO)
		return NULL;
		//bFile = FALSE;
	}	
...
{
	// TODO: Add your command handler code here
	ACTS1000_ListDeviceDlg(m_hDevice); // 列表该类设备
}
