...
void CADStatusView::OnInitialUpdate() 
{
	...
	str.Format(_T("%d"), m_TriggerVolt);
	m_nTringgerVolt.SetWindowText(str);

	m_nScreenVolume.SetCurSel(12);
//	gl_ScreenVolume = 10000;

	m_nMoveVolt.SetWindowText(_T("0"));
	
	m_nAnalyzeCount.SetWindowText(_T("0"));
	
	m_nOverRange.SetWindowText(_T("0"));//要返回的
	
	m_nOverRatio.SetWindowText(_T("0"));//要返回的
	if(gl_bTileWave)
	{
...
void CADStatusView::OnChangeEDITTriggerVolt() //改变触发电压的值
{
	...
	m_TriggerVolt=_tcstol(str,NULL,10);
...
void CADStatusView::OnSelchangeComboScreenVolume() 
{
	...
	gl_ScreenVolume=_tcstol(str,NULL,10);
