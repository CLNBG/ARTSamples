...
int CADFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	{ // 这个视图没有问题
		pParaCfgView=(CParaCfgView*)m_pWndSTCBar->AddView(_T("参数配置"), RUNTIME_CLASS(CParaCfgView/*CParaCfgView*/));//CParaCfgView
		pParaCfgView->m_pADFrm = this;
	}
	{// 这个视图有有问题
		pADStatusView=(CADStatusView*)m_pWndSTCBar->AddView(_T("性能分析"), RUNTIME_CLASS(CADStatusView));//CADStatusView
	}	
	...
	if(pParaCfgView != NULL)
	{
		...
		/*m_ToolTip.AddTool(pFrequency,_T("设定频率"));
		m_ToolTip.AddTool(pTriggerSource,_T("内部触发或外部触发"));
		m_ToolTip.AddTool(pTriggerDir,_T("触发方向"));
		m_ToolTip.AddTool(pBufferID,_T("在2个缓冲区中选择缓冲区"));
		m_ToolTip.AddTool(pSLIDER_Offset,_T("在缓冲区中选择偏移"));*/
	}
	return 0;
}
...
// 			return;
		...
		if (lCount>8192)	// 8192为SHORT DataAnalyze[MAX_CHANNEL_COUNT][8192];
		{
			lCount = 8192;
		}
		...
		/////////////////////////////////////////////			
		for(int k=0; k<MAX_CHANNEL_COUNT; k++)//每通道的数据各自进行排序
		{
			...
			// m_DataMiddle是非偏移码，即原码，即0-gl_fLsbCount中的任意值
			// gl_MiddleLsb 是偏移码值，即原码减去gl_lLsbHalf后的值,即在原点上下浮动的值
